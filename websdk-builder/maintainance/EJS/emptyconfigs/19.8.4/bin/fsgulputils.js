...
module.exports = (function() {
  ...
  ctx.parseGWConfigFile = function(fileloc) {
    ...
    if (fs.existsSync(externprefix + "/clientcode/" + codever + "/src/version.json")) {
      var vfile = fs
        .readFileSync(externprefix + "/clientcode/" + codever + "/src/version.json")
        .toString();
      if (vfile) {
  ...
  ctx.zipUp = function(cb, proj) {
    ...
    zipArchive.bulk([{ src: ["**/*"], cwd: srcDirectory, expand: true }]);

    zipArchive.append(null, { name: "gateway/" });

    var gatewayconfig = fs
      .readFileSync(clientProperties.build.config + "/" + proj + "/gateway_config.js")
      .toString("utf-8");
    gatewayconfig = gatewayconfig.replace(/\${versionTag}/gi, pjson.version);
    gatewayconfig = gatewayconfig.replace(
      /\${recTransporturl}/gi,
      clientProperties.client.transporturl
    );
    gatewayconfig = gatewayconfig.replace(fnameReg, "fs.$1.v" + codeversion + ".js");
    gatewayconfig = Buffer.from(gatewayconfig, "utf-8");
