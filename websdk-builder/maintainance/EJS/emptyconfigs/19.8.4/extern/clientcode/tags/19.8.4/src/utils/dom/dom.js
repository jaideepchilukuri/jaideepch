...
utils.addClass = function(elements, value) {
  var i, l, j, elem, curr_class_name;
...
utils.css = function(elm, opts, forceStyle) {
  if (elm) {
    if (!fs.isDefined(elm.length)) elm = [elm];
...
utils.restrictFocus = function(elem) {
  ...
  var elements_to_focus_on = document
    .querySelectorAll(
      "a, input[type=text], textarea, button, input[type=radio], select, *[tabIndex]",
      elem
    )
    .sort(function(e1, e2) {
          ...
          if (elements_to_focus_on[i] === e.target) {
            ...
            if (e.shiftKey) {
              do {
                ...
                (elements_to_focus_on[j].offsetLeft <= 0 || elements_to_focus_on[j].tabIndex < 0) &&
                j != i
              );
