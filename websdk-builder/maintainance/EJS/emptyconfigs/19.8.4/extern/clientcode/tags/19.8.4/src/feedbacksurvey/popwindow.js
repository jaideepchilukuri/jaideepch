  ...
  if (cfg) {
    ...
    this.$content = $(
      '<div id="acsMainContentDialog" class="acsMainContainerMobile--' +
        cfg.template +
        ie10Class +
        '" role="dialog" aria-labelledby="acsFeedbackDialogTitle" aria-describedby="acsFeedbackDialogDesc" aria-live="assertive"></div>'
    );
...
PopWindow.prototype._getCSS = function(prom) {
  ...
  /* pragma:DEBUG_END */
  utils.loadCSS(
    _fsNormalizeUrl("$templates/feedback/" + (this.sv.cfg.template || "default") + "/main.css"),
    function(link) {
      if (link) {
        /* pragma:DEBUG_START */
