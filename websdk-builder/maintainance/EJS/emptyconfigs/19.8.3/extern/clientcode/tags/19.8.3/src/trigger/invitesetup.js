...
var getInviteSetupInstance = function(
  trig,
  browser,
  stg,
  cpps,
...
var InviteSetup = function(
  ...
  if (
    fs.isDefined(this.trig.surveydef.inviteExclude) &&
    fs.isDefined(this.trig.crit) &&
  ...
  fsReady(function() {
    ...
    var display = InviteSetup.getFittingDisplay(
      trig.surveydef,
      displayoverride,
      cpps.get("locale"),
      browser
    ...
    if (display.inviteType === "SHORTSURVEY") {
      ...
      require([fs.makeURI("$fs.presenter.js")], function(Presenter) {
        ctx.invite = trig.invite = new Presenter(config, trig.surveydef, display, cpps);
...
InviteSetup.prototype.initialize = function() {
  ...
  invite.declined.subscribe(function(decline_type) {
    ...
    var rDays =
      fs.isDefined(config.active_surveydef) && fs.isDefined(config.active_surveydef.repeatDays)
  ...
  invite.abandoned.subscribe(function() {
    /* pragma:DEBUG_START */
    console.warn(
      "trigger: user abandoned the invitation. going to now wait " +
        Math.round(config.config.reinviteDelayAfterInviteAbandon / 1000) +
...
InviteSetup.prototype.present = function() {
  ...
  this.resourcesready.subscribe(
    function() {
      var inviteDelay = config.config.inviteDelay;
      ...
      /* pragma:DEBUG_START */
      console.log(
        "trigger: delaying invite for ",
        Math.max(0, inviteDelay - (utils.now() - fs.startTS)),
        " ms"
      );
      /* pragma:DEBUG_END */
 ...
 */
InviteSetup.getFittingDisplay = function(surveyDef, displayoverride, locale, browser) {
