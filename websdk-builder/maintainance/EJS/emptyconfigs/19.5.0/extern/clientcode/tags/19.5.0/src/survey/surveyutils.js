...
(function () {
  ...
  surveyUtils.cleanUpText = function (blob, maxlen) {
    ...
    if (blob.length > 9) {
      ...
      // Save whitelisted info from censorship, to restore it later
      var whitelisted = [];
      for(var regexw in rew) {
        blob = blob.replace(rew[regexw], function (match, index) {
          this.push({i:index, m:match});
          return '';
        }.bind(whitelisted) );
      }

      var maskStr = function (match) { return new Array(match.length+1).join('X'); };

      // Censor sensitive data following the above regexes
      for (var regexb in reb) {
