...
const ___log = () => {
   ...
   */
  class Logger {
    constructor(fs, Compress, Differ, tree, censor, EVENT_TYPES, payloadReadyCB, partialCB) {
      fs.ext(
        this,
        {
          Differ,
      ...
      arr.forEach(evt => {
        if (evt.d) {
          ...
          if (evt.e == this.types.PAGE_MARKER && evt.d.doc) {
            ...
            const diff = this.Differ(evt.d.v0, evt.d.v1);
    ...
    pinch() {
      ...
      if (this.log.length > 0) {
        ...
        const payload = this.Compress.compress(JSON.stringify(this.log));
