...
class Invite {
  ...
  _handleAcceptCurrentStage() {
    const acsMain = document.getElementById("acsFullScreenContainer");
    switch (this.display.inviteType) {
      case "TRACKER":
  ...
  _trapKeyBoardMobile(acsMain, dialog, ff) {
    ...
    if (this.inviteStage === 0) {
      ...
      containConditional = target => {
        ...
        return (!DOMContains(dialog, target) && !DOMContains(exitBtn, target)) || !acsMain;
      };
    } else if (this.inviteStage === 1) {
   ...
   */
  _undoAriaHide() {
    const hiddens = this.ariaHiddens;
