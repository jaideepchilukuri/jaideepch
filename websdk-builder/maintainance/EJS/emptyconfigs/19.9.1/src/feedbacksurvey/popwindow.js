...
class PopWindow {
  constructor(configs, browser, jrny, mid, cppo, namespace, isstandalone) {
    ...
    if (cppo) {
      ...
      if (!isObject(cppo) && cppo.length > 2) {
        ...
        cppo = JSON.parse(Compress.decompress(cppo));
      }
  ...
  _showThankyou() {
    const ctx = this;
    const res = Templater(this.epTemplate, this.survey);
  ...
  renderSurvey() {
    ...
    const res = Templater(this.svContentsTemplate, this.survey);
    this.$content.innerHTML = res;
  ...
  _postSurveyData(cb) {
    ...
    if (this.cfg.preview) {
      ...
      const tdata = Compress.compress(JSON.stringify(ext({ rating: this.sv._getScore() }, data)));
