...
Upgrader.prototype._upgrade18x = function (fullSrcPath) {
  ...
  if (fsgutil.fileExistsSync(fullSrcPath + "/client.properties.js")) {
    ...
    if (FSR) {
      ...
      // True conversion
      if (props.events && props.events.enabled) {
        var otc = props.events,
          ntc = {
            enabled: true,
            pd: otc.pd,
            codes: {}
          },
          ckeys = Object.keys(otc.custom);

        for (var i = 0; i < ckeys.length; i++) {
          var cval = otc.custom[ckeys[i]],
            nval = {
              code: otc.codes[ckeys[i]],
              repeat: cval.repeat,
              source: cval.source
            };
          if (cval.patterns) {
            nval.patterns = cval.patterns;
          }
          if (cval.name) {
            nval.name = cval.name;
          }
          ntc.codes[ckeys[i]] = nval;
        }


        prodcfg = prodcfg.replace(/\/\*[,\W]*trueconversion[\w\W\d]*}\*\//gi, ",trueconversion: " + fsgutil.JSONStringifyWithRegexp(ntc));

        ctx._logStep("Found true conversion. Converted to " + fsgutil.compressJSString(fsgutil.JSONStringifyWithRegexp(ntc)));
      }
