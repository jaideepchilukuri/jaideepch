...
(function () {
  ...
  Invite.prototype._validateMobileInput = function (successcallback) {
    var val = document.getElementById('acsEmailSMSInput').value,
      validateEmail = function (email) {
        ...
        input.setAttribute("aria-describedby", "acsInvalidInput");
        input.focus();
  ...
  Invite.prototype._switchToThankYouPage = function () {
    ...
    if (!!acsMain) {
      ...
      doc.getElementById("acsTYHeading").focus();
  ...
  Invite.prototype._trapKeyBoardMobile = function (acsMain, dialog, ff) {
    ...
    if (this.inviteStage === 0) {
      ...
      containConditional = function (target) {
        ...
        return ((!utils.DOMContains(dialog, target) && !utils.DOMContains(exitBtn, target)) || !acsMain);
      };
    } else if (this.inviteStage === 1) {
  ...
  Invite.prototype.present = function () {
    ...
    if (this.isCustom) {
      ...
      if (displayopts.style.invite.backgroundImage) {
        displayopts.style.invite.backgroundImage = fs.makeURI("$" + displayopts.style.invite.backgroundImage);
      }
      if (displayopts.style.tracker.backgroundImage) {
        displayopts.style.tracker.backgroundImage = fs.makeURI("$" + displayopts.style.tracker.backgroundImage);
      }
