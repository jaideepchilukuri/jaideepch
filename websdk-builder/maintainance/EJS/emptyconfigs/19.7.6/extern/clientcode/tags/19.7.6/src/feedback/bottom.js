          ...
          if (crit.platformOK()) {
            ...
            for (var i = 0; i < configInstances.length; i++) {
              ...
              if (survey.TopicTester(inst)) {
                ...
                /* pragma:DEBUG_START */
                if (!didpass) { console.log("fb: failed Topics-Level whitelist/blacklist check", inst.mid, inst.topics); }
                /* pragma:DEBUG_END */
              } else {
                /* pragma:DEBUG_START */
              ...
              if ((didpass || (inst.topics && inst.topics.length === 0)) && !inst.disabled && (window == window.top)) {
                ...
                inst.jrny = new utils.Journey({
                  customerId: fs.config.customerId || utils.getRootDomain() || 'feedback_customerId',
