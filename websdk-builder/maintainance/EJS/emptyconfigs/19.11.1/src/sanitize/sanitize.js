import { isString, isObject, isArray, hasProp } from "../fs/index";

import { freeze, addToSet, clone, apply } from "./utils";
...
const { DATA_ATTR, ARIA_ATTR, IS_SCRIPT_OR_DATA, ATTR_WHITESPACE } = EXPRESSIONS;
...
const DEFAULT_ALLOWED_TAGS = addToSet({}, [].concat(TAGS.html, TAGS.text));

/* Allowed attribute names */
let ALLOWED_ATTR = null;
const DEFAULT_ALLOWED_ATTR = addToSet({}, [].concat(ATTRS.html, ATTRS.xml));
...
const _parseConfig = function(cfg) {
  ...
  URI_SAFE_ATTRIBUTES =
    "ADD_URI_SAFE_ATTR" in cfg ? addToSet({}, cfg.ADD_URI_SAFE_ATTR) : DEFAULT_URI_SAFE_ATTRIBUTES;
  ...
  /* Parse profile info */
  if (USE_PROFILES) {
    ALLOWED_TAGS = addToSet({}, [].concat(TAGS.text));
    ALLOWED_ATTR = [];
...
if (isSupported) {
  (function() {
    try {
      const doc = _initDocument('<svg><p><textarea><img src="</textarea><img src=x onerror=1//">');
  ...
  if (!ALLOWED_TAGS[tagName] || FORBID_TAGS[tagName]) {
    ...
    if (
      ...
      try {
        ...
        currentNode.insertAdjacentHTML("AfterEnd", htmlToInsert);
      } catch (error) {}
    }
...
/**
 * Recursively sanitize an object and it's children, mainly useful
 * for then providing this object to templates.
 */
  ...
  function recurse(obj) {
    if (isString(obj)) {
      ...
      for (let i = 0; i < obj.length; i++) {
        res.push(recurse(obj[i]));
      }

      return res;
    } else if (isObject(obj)) {
      // disallow recursive cyclical referances
      if (visited.has(obj)) {
        return {};
      }
      visited.add(obj);
      ...
      // allow non-recursive duplicate copies to be sanitized
      visited.delete(obj);
