...
try {
  ...
  if (taskName.indexOf('test_deploy_self_hosted_stg') > -1) {
    skipServer = true;
  }
...
gulp.task('push_stg_config', ['_pullextern'], function (cb) {
  FcpClient.promptForFCPCredentials({
    ...
    console.log("About to push a new default global configuration. Here it is:".magenta);
    defCfg.products.selfhosted = false;
    console.log(defCfg);
    /**
     * Set up an instance of the FCP client
     * @type {FCPClient}
...
/**
 * Debug version of self-hosted for testing
 */
gulp.task('test_deploy_self_hosted_stg', ['get_self_hosted_debug'], function (cb) {
  var env = "staging";
  FcpClient.promptForFCPCredentials({
    notes: true
  }, function (rs) {
    ...
    fcp.getClient(clientProperties.client.clientid, function (success, client) {
      if (!success) {
        ...
        fcp.makeContainer(clientProperties.client.sitekey, env, client.id, rs.notes, function (success, message) {
          if (!success && message !== "Container name already exists") {
            console.log("Failed to create container: ".red + message.red, env);
          } else {
            console.log(("Pushing self-hosting code to " + (/push_prod/.test(taskName) ? "production" : " staging") + " for client " + clientProperties.client.clientid + ":").magenta, client.name.yellow, "on site key".magenta, clientProperties.client.sitekey.yellow, "...".magenta);

            var buffer = fs.readFileSync('./dist/' + clientProperties.client.sitekey + "_selfhost.zip");
            fcp.pushCustomerConfigForProduct(
              ...
              function (success, result) {
                if (!success) {
                  console.log("Failed to upload product selfhosted:", result);
                  return;
                } else {
                  console.log("Successfully uploaded product selfhosted.".green, result);
                  process.nextTick(cb);
                }
              }, false, "{}");
 ...
 * Debug version of self-hosted for testing
 */
gulp.task('get_self_hosted_debug', ['get_self_hosted'], function (cb) {
  process.nextTick(cb);
});
        ...
        if (howmany == 0) {
          ...
          zipdir('./dist', function (err, buffer) {
            if (err) {
              ...
              console.log("Wrote file to ", outfile, "... (" + buffer.length + " bytes)");
              if (!skipServer) {
                handydevserver(
                  pjson.build.ports[1],
                  ['./dist'],
