...
(function (trigger) {
  ...
  var Tracker = function (template, def, cfg, stg, cpps, display, browser) {
    ...
    // Set the URL
    var enc = fs.enc;
    this._url = fs.makeURI("$fs.tracker.html?uid=" + enc(stg.uid || '') + "&sitekey=" + enc(fs.config.siteKey) + "&domain=" + enc(utils.getRootDomain()) + "&stg=" + enc(this.stg.pers) + "&gw=" + enc(fs.makeURI("trigger/__gwtest__")) + "&brain_url=" + enc(fs.config.brainUrl) + "&fsrlocale=" + enc(cpps.get('locale') || 'en') + "&_svu_=" + enc(fs.config.surveyUrl) + "&_cv_=" + enc(fs.config.codeVer) + '&_issh_=' + enc(fs.isSelfHosted) + "&_vt_=" + enc(fs.tagVersion) + "&_au_=" + enc(fs.config.analyticsUrl) + '&_pa_=' + enc(fs.assetLocation));
