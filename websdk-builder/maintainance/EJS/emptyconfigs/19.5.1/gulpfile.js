...
gulp.task('_html', function () {
  var logoimg = fs
    .readFileSync('./assets/p_b_foresee.svg')
    .toString();
  gulp
    .src(['./assets/**/*.html'])
...
gulp.task('_clientassets', function () {
  var productsListCopy = clientProperties
    .client
    .productsToBuild
    .slice(0);
...
gulp.task('_templates', function (cb) {
  ...
  for (var p = 0; p < templateproducts.length; p++) {
    var srcpath = './templates/' + templateproducts[p] + '/',
      dirs = fs
        ...
        .filter(function (file) {
          return fs
            .statSync(path.join(srcpath, file))
            .isDirectory();
        }),
      i,
      promiselist = [],
    ...
    // Set up the promises
    myProm
      .makePromises(promiselist, function (prod) {
        return function () {
    ...
    for (i = 0; i < dirs.length; i++) {
      ...
      gulp.src(srcpath + dirs[i] + '/sass/**/*.scss').pipe(sass().on('error', function () {
      ...
      gulp.src(srcpath + dirs[i] + '/html/**/*.*').pipe(gulpjsonp({
        callback: "__fsJSONPCB",
        key: ('templates/' + templateproducts[p] + '/' + dirs[i] + "/")
          .split('/')
          .join('_')
      }))
        .pipe(gulp.dest(destfiles))
...
gulp.task('_gateway', function (cb) {
  let gwDistFolder = pjson.build.extern + '/gateway/tags/' + pjson.gateway_version + '/dist';
  if (!fs.existsSync(gwDistFolder)) {
...
gulp.task('sdk', [
  ...
  runSequence([
    '_templates', '_customtemplates', '_images', '_html', '_clientassets'
  ], function () {
    ...
    fs.writeFileSync(distloc + '/fs.gateway.js', fs.readFileSync(pjson.build.extern + '/gateway/tags/' + pjson.gateway_version + '/dist/gateway' + (isProd
      ? '.min'
      : '') + '.js', 'utf-8'));
...
*/gulp.task('test_debug', [
  ...
  var textfilehandler = function (filename, contents, headers) {
    ...
    serverDirs = [
      './dist', './smoketest', pjson.build.extern + '/gateway/tags/' + pjson.gateway_version + '/dist'
    ],
    ...
    startServerSSL = function () {
      handydevserver(pjson.build.ports[1], serverDirs, {
        ssl: true,
        ignore: [
          'DS_Store', '_selfhost_', 'embed.txt', '.zip'
        ],
        latency: 250,
    ...
    fcp.listClients(function (success, data) {
      if (!success) {
        ...
        for (var i = 0; i < data.length; i++) {
          ...
          console.log("   Client: ".magenta, "[ID: ".grey + client.id.toString().yellow + "]".grey, client.name, (client.deleted != 0)
            ? "DELETED".red
            : "");
        ...
        fcp.lookupClient(res.client_search_term, function (success, data) {
          if (!success) {
            ...
            for (var i = 0; i < data.sites.length; i++) {
              ...
              console.log("   Client: ".magenta, "[ID: ".grey + site.client_id.toString().yellow + "]".grey, (site.deleted != 0)
                ? "DELETED".red
                : "");
            }
          }
...
gulp.task('create_client', function (cb) {
  ...
  console.log("Metadata can be the website URL, client contact name, other trademarks, etc. Thi" +
    "s is useful for searching.");
        ...
        if (!success) {
          ...
          fsGulpUtil.promptForValuesIfNeeded({
            ...
            if (res.sitekey && res.sitekey.length > 0) {
              ...
              fcp
                .doesSiteKeyExist(res.sitekey, function (success, exists, clientinfo) {
                  if (!success) {
  ...
  zipdir(distloc, function (err, buffer) {
    if (err) {
      ...
      FcpClient.promptForFCPCredentials({
        notes: true,
        // whether or not we prompt for 'latest'; true, unless we are using one of the
        // bash scripts
...
var getDefGlobalCfg = function (env) {
  ...
  if (env == FcpClient.environments.dev) {
    console.log("FCP environment:".magenta, "DEV".yellow);
    defCfg.brainUrl = defCfg
      .brainUrl
      .replace(/brain/, 'dev-brain');
    defCfg.surveyUrl = defCfg
      .surveyUrl
      .replace(/survey/, 'survey-dev');
    defCfg.recUrl = defCfg
      .recUrl
      .replace(/rec/, 'dev-rec');
    defCfg.analyticsUrl = defCfg
      .analyticsUrl
      .replace(/analytics/, 'dev-analytics');
  } else if (env == FcpClient.environments.qa) {
    console.log("FCP environment:".magenta, "QA".yellow);
    defCfg.brainUrl = defCfg
      .brainUrl
      .replace(/brain/, 'qa-brain');
    defCfg.surveyUrl = defCfg
      .surveyUrl
      .replace(/survey/, 'survey-qa');
    defCfg.recUrl = defCfg
      .recUrl
      .replace(/rec/, 'qa-rec');
    defCfg.analyticsUrl = defCfg
      .analyticsUrl
      .replace(/analytics/, 'qa-analytics');
     ...
     */
      ...
      fcp.makeContainer(clientProperties.client.sitekey, 'staging', clientProperties.client.clientid, rs.notes, function (success, message) {
        if (!success && message !== "Container name already exists") {
          ...
          fcp
            .postDefaultConfigForSiteContainer(clientProperties.client.sitekey, "staging", JSON.stringify(defCfg), "Posted a new default configuration (" + pjson.version + ")", function (success, result) {
              if (!success) {
                console.log("Was not able to connect. Message: ".red, result);
              } else {
                console.log("Success: ".magenta, result);
     ...
     */
      ...
      fcp.makeContainer(clientProperties.client.sitekey, 'production', clientProperties.client.clientid, rs.notes, function (success, message) {
        if (!success && message !== "Container name already exists") {
          ...
          fcp
            .postDefaultConfigForSiteContainer(clientProperties.client.sitekey, "production", JSON.stringify(defCfg), "Posted a new default configuration (" + pjson.version + ")", function (success, result) {
              if (!success) {
                console.log("Was not able to connect. Message: ".red, result);
              } else {
                console.log("Success: ".magenta, result);
...
gulp.task('get_self_hosted', ['sdk'], function (cb) {
  FcpClient
    .promptForFCPCredentials({}, function (rs) {
...
var BuildAndPushToEnv = function (environment, cb) {
  ...
  clientProperties.hasreplay = globalConfig
    .products
    .record
    .toString();
  ...
  FcpClient.promptForFCPCredentials({
    ...
    fcp.getClient(clientProperties.client.clientid, function (success, client) {
      if (!success) {
        ...
        fcp
          .makeContainer(clientProperties.client.sitekey, environment, client.id, rs.notes, function (success, message) {
            if (!success && message !== "Container name already exists") {
              console.log("Failed to create container: ".red + message.red, rs.environment);
            } else {
              console.log(("Pushing code to " + (/push_prod/.test(taskName)
                ? "production"
                : " staging") + " for client " + clientProperties.client.clientid + ":").magenta, client.name.yellow, "on site key".magenta, clientProperties.client.sitekey.yellow, "...".magenta);
              ...
              if (globalConfig.products.trigger) {
                ...
                // fs.writeFileSync(pjson.build.dist + '/trigger/config.json',
                // triggerConfigJSON);
                ...
                zipdir(pjson.build.dist + '/trigger', function (err, buffer) {
                  if (err) {
                    ...
                    fcp
                      .pushCustomerConfigForProduct(clientProperties.client.clientid, clientProperties.client.sitekey, environment, "trigger", triggerGWConfig, buffer, rs.notes, function (success, result) {
                        if (!success) {
              ...
              if (globalConfig.products.record) {
                ...
                zipdir(pjson.build.dist + '/record', function (err, buffer) {
                  if (err) {
                    ...
                    fcp
                      .pushCustomerConfigForProduct(clientProperties.client.clientid, clientProperties.client.sitekey, environment, "record", recordGWConfig, buffer, rs.notes, function (success, result) {
                        if (!success) {
              ...
              if (globalConfig.products.feedback && !skipPublishingFeedbackConfig) {
                ...
                zipdir(pjson.build.dist + '/feedback', function (err, buffer) {
                  if (err) {
                    ...
                    fcp
                      .pushCustomerConfigForProduct(clientProperties.client.clientid, clientProperties.client.sitekey, environment, "feedback", feedbackGWConfig, buffer, rs.notes, function (success, result) {
                        if (!success) {
  ...
  BuildAndPushToEnv(container, function (fcpRes) {
    ...
    fsGulpUtil.promptForValuesIfNeeded(emailOpts, function (res) {
      if ((res && res.sendCodeEmail == 1) || (envVariableOptions && envVariableOptions.email && envVariableOptions.email.toString() == '1')) {
        var recipient = creds
          .username
          .substr(0, creds.username.indexOf('@')) + "@foresee.com";
...
gulp.task('promote_prod', function (cb) {
  FcpClient
    .promptForFCPCredentials({
      ...
      var fcp = new FcpClient(rs.username, rs.password, rs.environment);
      fcp.promoteStgToProd(clientProperties.client.sitekey, rs.notes, [
        'trigger', 'record'
...
gulp.task('code_version_report', ['_pullextern'], function (cb) {
  ...
  FcpClient
    .promptForFCPCredentials({}, function (rs) {
      ...
      // create progress bar const ProgressBar = require('ascii-progress'); const bar =
      // new ProgressBar({   schema: '[:bar] :percent :elapseds/:etas' });
  ...
  function getDefaultCodeVersion(fcp) {
    return new Promise(function (resolve, reject) {
      fcp
        .getDefaultConfig(function (success, data) {
          if (!success) {
            resolve('default');
          } else {
  ...
  function listClients(fcp, bar, email, codeVer) {
    fcp
      .listClients(function (success, clients) {
        if (!success) {
          ...
          // execute these promises for each client serially instead of in parallel we are
          // using this as a throttling mechanism (active directory service can get
          // overwhelmed with too many concurrent auth requests)
          serial(promiseFunctions).then(function (report) {
  ...
  function sendReport(bar, email, report) {
    // bar.update(1);
    var transport = mailer.createTransport(smtpTransport({ host: "webmail.foreseeresults.com", port: 25, ignoreTLS: true }));
  ...
  function listContainersForSite(fcp, codeVer, siteObj, resolveSite, rejectSite) {
    fcp
      .getContainersForSitekey(siteObj.siteKey, function (success, containers) {
        if (!success) {
          ...
          if (containers.length) {
            ...
            let containerPromises = containers.map(function (container) {
              return new Promise(function (resolveContainer, rejectContainer) {
                ...
                Promise
                  .all([getCodeVersion, getProducts])
                  .then(function () {
                    resolveContainer();
                  })
                  .catch(function (e) {
