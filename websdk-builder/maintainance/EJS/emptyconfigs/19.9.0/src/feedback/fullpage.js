...
import { Bind, Unbind, FSEvent, getSize, getScroll, AjaxTransport } from "../utils/utils";
...
class FullPageSurvey {
  ...
  show() {
    ...
    if (this.$content.children.length === 0) {
      const res = Templater(this.svTemplate, this.sv);
  ...
  _showThankyou() {
    this._removeWait();
    const res = Templater(this.epTemplate, this.sv);
