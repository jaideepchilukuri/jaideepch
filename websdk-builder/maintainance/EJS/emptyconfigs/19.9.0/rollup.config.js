...
const BASEDIR = `src`;
const DISTDIR = `dist/code/${VERSION}`;
...
const components = [
  ...
  { name: "presenter", desc: "Short Survey Presenter", entry: "presenter.js" },
];

const fsModule = `${__dirname}/${BASEDIR}/fs/index.js`;
const utilsModule = `${__dirname}/${BASEDIR}/utils/utils.js`;
const surveyModule = `${__dirname}/${BASEDIR}/survey/survey.js`;

const componentBuildInstructions = components.map(({ name, desc, entry }) => ({
  input: `${name}/${entry}`,
  output: {
    name: name,
    file: `${DISTDIR}/fs.${name}.js`,
    ...
    globals: id => {
      if (id.includes("/fs/")) return "fs";
      if (id.includes("/utils/")) return "utils";
      if (id.includes("/survey/")) return "fsSurvey";
      return id;
    },
  },
  external: [
    ...
    "triggerconfig",
    "feedbackconfig",
    name === "gateway" ? "nevermind" : fsModule,
    name === "utils" ? "nevermind" : utilsModule,
  ...
  plugins: [
    ...
    conditional(name !== "gateway", [fsAmdWrapper(name)]),
...
function makeBanner(name, componentDesc) {
  ...
  return [
    ...
    ` * Version: ${VERSION}`,
    ` * Built: ${today}`,
    name === "utils" ? " * Contains Pako (c) nodeca/pako on github" : "",
    " ***************************************/",
  ]
...
function fsGatewayBuilder() {
  ...
  return {
    ...
    async writeBundle(bundle) {
      const fileName = `${DISTDIR}/fs.gateway.js`;
      const gwsrc = bundle["fs.gateway.js"].code;

      // put the built files for the gateway under the dist/gateway folder
      const gatewaymin = `${DISTDIR}/gateway.min.js`;
      const gateway = `${DISTDIR}/gateway.js`;
