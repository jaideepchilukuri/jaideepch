 ...
 * @constructor
 */
var DoPublish = function (u, p, env, notes) {
...
gulp.task("publish", ['code'], function (cb) {
  FcpClient.promptForFCPCredentials({
    ...
    if (testEnvs) {
      ...
      // Publish go the four testing environments
      Promise.all([
        DoPublish(rs.username, rs.password, fe[fes[0]], rs.notes),
        DoPublish(rs.username, rs.password, fe[fes[1]], rs.notes),
        DoPublish(rs.username, rs.password, fe[fes[2]], rs.notes),
        DoPublish(rs.username, rs.password, fe[fes[3]], rs.notes)
      ]).then((env => {
        console.log("Published to ", env);
        cb && cb();
      }));
    } else {
      // Publish only to the selected environment
      DoPublish(rs.username, rs.password, rs.environment, rs.notes).then((env) => {
        console.log("Published to ", env);
        cb && cb();
      });
    }
...
gulp.task("reset", ['code'], function (cb) {
  FcpClient.promptForFCPCredentials({}, function (rs) {
    ...
    fcp.reset(function () {
      ...
      setTimeout(function () {
        DoPublish(rs.username, rs.password, rs.environment, "Initial version").then((env) => {
          // console.log("Published to ", env);
          cb && cb();
        });
      }, 10000);
