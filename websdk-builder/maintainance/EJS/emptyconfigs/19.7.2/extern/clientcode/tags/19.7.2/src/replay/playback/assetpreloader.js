...
fs.provide("rp.Replay.Playback.AssetPreloader");
...
(function () {
  ...
  AssetPreloader.originalUrl = function (proxyUrl) {
    try {
      ...
      var encUri = paramStrs.
        find(function (s) { return s.indexOf('url=') === 0; }).
        split('=')[1];
      var encBase = paramStrs.
        find(function (s) { return s.indexOf('context=') === 0; }).
        split('=')[1];
  ...
  AssetPreloader.prototype.preloadAll = function (dataList) {
    ...
    var cssAssets = this.dedupeList(
      assetList.filter(this.isStylesheet)
    ).filter(this.hasNoDeviceMediaQuery);

    return this.fetchCSS(cssAssets, assetList).then(function () {
      ...
      assetList = assetList.filter(function (asset) {
        return asset.key !== "css-url" &&
          this.hasNoDeviceMediaQuery(asset);
      }.bind(this));

      console.log("Preloading", assetList.length, "assets:", assetList);

      return this.addPreloadLinks(assetList).then(function () {
  ...
  AssetPreloader.prototype.fetchCSS = function (cssAssets, assetList) {
    ...
    return Promise.all(cssAssets.map(function (asset) {
      return fetch(asset.url, {
        ...
        if (!res.ok) {
          if (!asset.failed) {
            ...
            console.error("FATAL: stylesheet preload failed: " + AssetPreloader.originalUrl(asset.url), asset);
          }
          return null;
        }
    ...
    return new Promise(function (resolve) {
      ...
      assets.forEach(function (asset) {
        ...
        var timeout = setTimeout(function () {
          console.error("FATAL Timed out preloading: " + AssetPreloader.originalUrl(asset.url), asset);
          done();
        }.bind(this), PRELOAD_TIMEOUT);
        var error = function () {
          if (!this.isImage(asset)) { // don't report imgs
            console.error("Error preloading: " + AssetPreloader.originalUrl(asset.url), asset);
          } else if (asset.key !== 'css-url') { // don't report bad css assets
  ...
  AssetPreloader.prototype.cssScan = function (text, list, urlType) {
    ...
    fonts.forEach(function (font) {
      var fontUrls = (font.match(/url\s*\(\s*['"]?\/replay\/proxy\?[^\)]+\)/g) || []).
        map(function (url) {
          return "/replay/proxy" + url.split("/replay/proxy")[1].replace(/['"]?\s*\)$/, '');
        });
      urls = urls.filter(function (u) { return fontUrls.indexOf(u) < 0; });

      // first try to use a the first woff2, woff or truetype font found
      var fontUrl = font.match(/url\s*\(\s*['"]?(\/replay\/proxy\?[^\)'"]+)['"]?\s*\)\s*format\(['"](woff2|woff|truetype)['"]\)/);
  ...
  AssetPreloader.prototype.domScan = function (initialDOM, events) {
    ...
    function search(node, list) {
      ...
      if (node.n === "STYLE" && node.c) {
        var text = node.c.map(function (txt) { return txt.v || ""; }).join("");
  ...
  AssetPreloader.prototype.isImage = function (asset) {
    return (asset.key === 'css-url' || asset.key === 'style-url') ||
