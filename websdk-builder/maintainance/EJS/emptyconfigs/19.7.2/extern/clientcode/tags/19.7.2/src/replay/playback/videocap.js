...
(function () {
  ...
  VideoCapture.prototype._setupMediaRecorder = function (stream) {
    ...
    this.recorder.addEventListener("start", function (evt) {
      this.blobs = [];
      this.timeOffset = 0;
      this.startTime = evt.timeStamp;
      this.recording = true;
    }.bind(this), false);
    this.recorder.addEventListener("stop", function (evt) {
      if (this.startTime > -1) {
    ...
    this.recorder.addEventListener("resume", function (evt) {
      if (this.startTime < 0) {
        this.startTime = evt.timeStamp;
      }
      this.recording = true;
    }.bind(this), false);
    this.recorder.addEventListener("pause", function (evt) {
      if (this.startTime > -1) {
        this.timeOffset += evt.timeStamp - this.startTime;
      }
      this.startTime = -1;
   ...
   * @private
   */
  VideoCapture.prototype._uploadScreenCapture = function (seq, data) {
    // serialize uploads
    if (this.uploading) {
      this.blobs.push({ seq: seq, data: data });
      return;
    }
    this.uploading = true;
    ...
    var done = function () {
      ...
      if (this.blobs.length > 0) {
        this.blobs.sort(function (a, b) { return a.seq - b.seq; });
        var next = this.blobs.shift();
    ...
    // upload
    var url = "http://localhost:" + this.uploadPort + "/upload/" + seq;
