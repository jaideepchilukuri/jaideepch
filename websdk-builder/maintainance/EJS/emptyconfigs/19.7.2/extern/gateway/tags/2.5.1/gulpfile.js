...
gulp.task('code', ['clean'], function (cb) {
  ...
  console.log("Building gateway ".grey + pJSON.version.toString().yellow + " with a default code version of ".grey + defaultCodeVer.toString().yellow + ".".grey);
  ...
  if (forceDebug) {
    ...
    return gulp.src(paths.gatewaysrc)
      ...
      .on('error', function (err) { console.error(err.toString().red); })
      .pipe(wrap(getLegal() + '\n<%= contents %>'))
      .pipe(gulp.dest(paths.dest));
  }
});
