...
try {
  ...
  if (taskName.indexOf('_all_env') > -1) {
    allEnvs = true;
  }
} catch (e) {
}
 ...
 */
gulp.task('push_code_all_env', ['push_code']);
...
gulp.task('push_stg', ['sdk'], function (cb) {
  ...
  BuildAndPushToEnv(env, function () {
    ...
    fsGulpUtil.promptForValuesIfNeeded(emailOpts, function (res) {
      if ((res && res.sendCodeEmail == 1) || (envVariableOptions && envVariableOptions.email && envVariableOptions.email.toString() == '1')) {
        ...
        fsGulpUtil.sendClientCodeEmail(clientProperties.client.id, clientProperties.client.sitekey, recipient, env, function () {
