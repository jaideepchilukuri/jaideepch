    ...
    browser.ready.subscribe(function () {
      ...
      stg.ready.subscribe(fs.proxy(function () {
        ...
        stg.upgradeOldStorage(function () {
          ...
          var jrny = Singletons._journey = new utils.Journey(config.config.id, utils.APPID.TRIGGER, stg.uid, browser);
                        ...
                        if (trig.doesPassCriteria()) {
                          ...
                          if (trig.surveydef) {
                            ...
                            if (invitesituation != 'x') {
                              ...
                              if (!browser.isMobile && !browser.isTablet && trig.surveydef.mouseoff && trig.surveydef.mouseoff.mode !== 'off') {
                                if (stg.get('pv') == 1) {
                                  stg.set('sst', utils.now());
                                }
                                // MouseOff is only available if first page's surveydef had it enabled
                                if (stg.get('sst')) {
                                  mo = trig.mouseoff = new MouseOff(trig, trig.surveydef, browser, stg, jrny);
                                }
                              }
                              ...
                              if (trig.canDisplayInvitation()) {
                                ...
                                if (trig.evalLoyaltySampling()) {
                                  ...
                                  var ist = getInviteSetupInstance(trig, browser, stg, cpps, false, jrny, function () {
                                    // Initialize and present the invitation
                                    this.initialize();
                                    this.present();
                                  });

                                } else if (mo && mo.checkCriteria()) {
                                  // Check the MouseOff criteria
                                  ...
                                  mo.startListening(function () {
                                    this.inviteSetup = getInviteSetupInstance(trig, browser, stg, cpps, false, jrny, function () {
                              ...
                              if (mbhi) {
                                ...
                                // If not mobile, update the tracker
                                ...
                                getInviteSetupInstance(trig, trig.browser, trig.stg, trig.cpps, false, trig.jrny, function () {
