...
fs.provide("rp.Replay.Playback.CssMod");

fs.require("rp.Top");
fs.require("rp.Replay.Playback.MediaQuery");
...
(function () {
  ...
  CssMod.prototype._updateFixedPositionElements = function (doc) {
    ...
    function finder(el) {
      return function (v) { return v.el === el; };
    }
    ...
    for (i = 0; i < this.fixedPositionInfo.length; i++) {
      ...
      for (j = 0; j < els.length; j++) {
        ...
        if (!elinfo) {
          elinfo = {
            enabled: false, doc: doc, el: el, origStyles: {
              left: el.style.left,
              top: el.style.top,
  ...
  CssMod.prototype._calculateFixedAnchor = function (elinfo) {
    ...
    ["top", "left", "bottom", "right"].forEach(function (prop) {
      if (styles.find(function (rule) {
        return rule.style.getPropertyPriority(prop) === "important" &&
    ...
    ["width", "height"].forEach(function (prop) {
      var rule = styles.find(function (rule) {
        return rule.style[prop] != null && rule.style[prop] !== "" && rule.style[prop] !== "auto" && rule.style[prop].match(percentMatcher);
      });
      if (!rule) return;
      var match = rule.style[prop].match(percentMatcher);
      if (match) {
    ...
    if (
      elinfo.top && elinfo.top.offset === 0 &&
      elinfo.left && elinfo.left.offset === 0 &&
      elinfo.bottom && elinfo.bottom.offset === 0 &&
      elinfo.right && elinfo.right.offset === 0
    ) {
  ...
  CssMod.prototype._positionFixedFixUnnecessary = function (doc) {
    ...
    this.disablePositionFixedFix = height &&
      (height.unit === 'percent' || height.unit === 'vh') &&
      height.value > 50;
  ...
  CssMod.prototype._onViewportUpdate = function (params, doc) {
    var sizeChange = this.viewportParams && (
      params.lw !== this.viewportParams.lw ||
      params.lh !== this.viewportParams.lh
    );
