...
fs.require("rec.Worker.RecordWorker");
fs.require("rec.Data.Session");
fs.require("rec.Data.Diff");
fs.require("rec.Capture.Actions");
fs.require("rec.Capture.Masker");
...
(function () {
   ...
   * @constructor
   */
  var Recorder = function (stg, browser, winObj, instancePath, parentInstance, config, domTree, isIframeMode, iFrameID, iFrameParentFr, iFrameIdPrefix, xIframePositionOffset, skipPage) {
    /* pragma:DEBUG_START */
    ...
    if (this.win.document) {
      ...
      this.sesh = (parentInstance && parentInstance.sesh) ? parentInstance.sesh : new RecordSession(stg, config);
      ...
      this.worker = (parentInstance && parentInstance.worker) ? parentInstance.worker : new RecordWorker(config, this.sesh, stg, this.masker);

      /* pragma:DEBUG_START */
      console.log("sr: " + (winObj === window.top ? "" : "[frm] ") + "logger ready on " + winObj.document.title);
      /* pragma:DEBUG_END */

      // Set up the capture stuff
      if (!skipPage) {
        this.cap = new Capture(browser, this.config, this.isIframeMode, this.getTop() === this, winObj, this.masker, this.worker, this, instancePath, this.urlmd5, domTree, this.sesh.sessionInfo);
      } else {
        /* pragma:DEBUG_START */
        console.log("sr: " + (winObj === window.top ? "" : "[frm] ") + "Skipping page b/c blacklist");
        /* pragma:DEBUG_END */
      ...
      /* pragma:DEBUG_START */
      console.warn("sr: " + (winObj === window.top ? "" : "[frm] ") + "record initialization complete on page: %c" + winObj.document.title, "color:green");
      /* pragma:DEBUG_END */
    }
  };
  ...
  Recorder.prototype.processImmediately = function (delay) {
    ...
    new utils.AjaxTransport().send({
      method: 'GET',
      url: utils.sign(fs.config.recUrl + 'process/' + fs.enc(this.getGlobalId()) + '?version=' + fs.enc(fs.config.codeVer == "symlink" ? "19.7.0" : fs.config.codeVer) + '&delay=' + (delay || 0)),
      failure: function (result) {
        /* pragma:DEBUG_START */
        console.warn("sr: " + (window === window.top ? '' : '[frm] ') + "Session processing request failed for global", this.getGlobalId(), "Note: this doesn't necessarily mean there is a problem. The processing may already have been started.");
        /* pragma:DEBUG_END */
      }.bind(this),
      success: function (result) {
        /* pragma:DEBUG_START */
        console.log("sr: " + (window === window.top ? '' : '[frm] ') + "Session processing started for global", this.getGlobalId());
        /* pragma:DEBUG_END */
      }.bind(this)
    });
  };
  ...
  Recorder.prototype.dispose = function () {
    /* pragma:DEBUG_START */
    console.log("sr: " + (window === window.top ? "" : "[frm] ") + "disposing Recorder instance. already disposed?", !!this.disposed);
    /* pragma:DEBUG_END */
