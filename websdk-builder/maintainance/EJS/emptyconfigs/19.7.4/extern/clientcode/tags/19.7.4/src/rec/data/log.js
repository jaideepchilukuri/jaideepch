...
(function () {
  ...
  var ___log = function () {
    ...
    Logger.prototype.addEvents = function (arr) {
      ...
      var containsPageMarker = false;

      // Apply the masking
      arr.forEach(function (evt) {
        if (evt.d) {
          // Dom Serialize event
          if (evt.e == this.types.PAGE_MARKER) {
            containsPageMarker = true;
      ...
      if (
        ...
        (!this.corked && (this.dataSize >= this.dataSizeGoal || containsPageMarker))
      ) {
        this.pinch(containsPageMarker);
      }
    };
     ...
     * @param {Array} arr
     */
    Logger.prototype.pinch = function (containsPageMarker) {
      // Compress
      if (this.log.length > 0) {
        ...
        if (!this.firstPacket && !containsPageMarker) {
