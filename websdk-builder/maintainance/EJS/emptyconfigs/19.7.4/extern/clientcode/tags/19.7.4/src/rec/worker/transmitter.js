...
(function () {
  ...
  Transmitter.prototype._buildUrl = function () {
    ...
    return fs.config.recUrl +
      ...
      fs.enc(this.sesh.sessionInfo[SESSION_SYMBOLS.GLOBALSESSIONID]) + "/" +
      fs.enc(this.sesh.sessionInfo[SESSION_SYMBOLS.SESSIONID]) +
  ...
  Transmitter.prototype._actuallyTransmit = function () {
    ...
    if (pls.length > 0 && !this._transmitInFlight) {
      this._transmitInFlight = true;

      /* pragma:DEBUG_START */
      var bytes = JSON.stringify({ data: pls }).length;
      this._totalBytes = (this._totalBytes || 0) + bytes;

      console.log('sr: ' + (window === window.top ? '' : '[frm] ') +
        '>>> actually sending ' + pls.length + ' packets to the server (' + bytes + ' b out of ' + this._totalBytes + ' b total) >>>');
      /* pragma:DEBUG_END */
