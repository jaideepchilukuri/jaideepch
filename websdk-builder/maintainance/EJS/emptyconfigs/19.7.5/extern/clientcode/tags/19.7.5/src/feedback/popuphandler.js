...
(function () {
  ...
  PopupHandler.initialize = function (cfg, browser, cpps, errortemplate, modaltemplate, eptemplate) {
    ...
    if (!!pu && !pu.cfg.popup) {
      ...
      pu.SurveySubmitted.subscribe(function (cg) {
        return function () {
  ...
  PopupHandler.computeSurveyType = function (configuredSurveyType) {
    ...
    if (configuredSurveyType !== 'fullpage' && (Singletons.browser.isMobile || utils.getSize(window).w <= 500)) {
      return "popup";
    }
