...
function startTestServer(pjson, clientProperties, isSSL) {
  ...
  const textfilehandler = function(filename, contents) {
    ...
    if (filename.indexOf("gateway.js") > -1 || filename.indexOf("gateway.min.js") > -1) {
      ...
      for (const product of configs.products) {
        if (
          configs.global.products[product] ||
          (product === "fbmods" && configs.global.products.feedback)
        ) {
          const json = JSON.stringify(configs[product], null, 2);
          productConfigStr += `productConfig.${product} = ${json};\n\n`;
        }
      }
  ...
  try {
    fs.symlinkSync(`./${pjson.version}`, path.resolve("./dist/code/current"), "dir");
    serverDirs.push("./dist/code/current");
  } catch (e) {
    console.error(
      "Warning: proxying to /code/current/* won't work because symlink creation failed"
    );
  }
  ...
  const startServer = function() {
    handydevserver(pjson.build.ports[0], serverDirs, {
      ssl: false,
      ignore: ["DS_Store", "_selfhost_", "embed.txt", ".zip"],
      latency: process.argv.includes("--test") ? 0 : 250,
      ontextfile: textfilehandler,
    });
