...
const { hashBuffer } = require("./integrity");
...
function buildSelfHosted(pjson, clientProperties, debug, rs) {
  optionallyPromptForCredentials(rs, {}, function(rs) {
    ...
    let integritySnippet = fs
      .readFileSync("./bin/integritysnippet.html")
      .toString()
      .split("\n")
      .join("\n    "); // re-indent
    ...
    gatewayFile = gatewayFile.toString().replace(
      ...
      `globalConfig = ${JSON.stringify({
        ...
        gateway,
        configLocation,
        integrityHashLocation: clientProperties.client.integrityHashLocation || null,
      })}\n`
    );
    ...
    if (debug) {
      ...
      delete configs.products;

      const text = JSON.stringify(configs, null, 2);

      // for testing the config.json integrity hash feature
      const hash = hashBuffer("sha256", Buffer.from(text, "utf8"));
      integritySnippet = integritySnippet.replace(/\[\]/g, `["${hash}"]`);

      fs.writeFileSync("./dist/config/staging/config.json", text);
      fs.writeFileSync("./dist/config/production/config.json", text);
    }

    if (!clientProperties.client.integrityHashLocation) {
      // if the feature is off, don't bother putting the example
      // snippet into the html
      integritySnippet = "";
    }

    sampleHTMLFile = sampleHTMLFile.replace(/\{\$ver\}/g, pjson.version);
    sampleHTMLFile = sampleHTMLFile.replace(
      /\{\$embed\}/g,
      embedSnippet.split("\n").join("\n    ") // re-indent
    );
    sampleHTMLFile = sampleHTMLFile.replace(/\{\$integrity\}/g, integritySnippet);
