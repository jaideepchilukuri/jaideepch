...
class Invite {
  ...
  present() {
    ...
    const restemplate = this.invitetemplate(displayopts);
    ...
    const toggleBtnHandler = evt => {
      ...
      if (kC !== "escape") {
        toggleClass(submitBtn[0], "acsClassicInner--btn__grey", val.length < 1);
      }
    ...
    if (acsMain) {
      ...
      Bind(input, "invite:input", debounce(toggleBtnHandler, 100));
    }
