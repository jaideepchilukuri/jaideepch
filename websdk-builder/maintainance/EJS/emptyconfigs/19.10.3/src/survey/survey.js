...
class Survey {
   ...
   */
  _getScore() {
    return this.qs[0]._getRating ? this.qs[0]._getRating() : 0;
  }
  ...
  _validateSurvey() {
    let valid = true;
    let currentQuestion;
    let topInvalidQuestion;

    if (this.qs && this.qs.length > 0) {
      for (let i = 0; i < this.qs.length - 1; i++) {
        currentQuestion = this.qs[i].el;
        if (!this.qs[i].validate()) {
          ...
          if (!topInvalidQuestion) {
            topInvalidQuestion = this.qs[i].el;
          }
        } else {
  ...
  bind(nd) {
    ...
    const el = nd.$(".acs-feedback__block");
    for (let i = 0; i < el.length; i++) {
      const cfg = this._getQConfig(el[i].getAttribute("questionid"));
      if (cfg) {
        const q = buildQuestionType(el[i], cfg);
