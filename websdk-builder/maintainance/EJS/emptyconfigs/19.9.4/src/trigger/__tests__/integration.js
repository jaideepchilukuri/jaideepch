...
import { JSONP } from "../../utils/network/jsonp";
import fs from "fs";
...
jest.mock("../../utils/network/jsonp");

const inviteTemplateHtml = fs
  .readFileSync(path.resolve("templates/trigger/desktopredesign/html/invite.html"))
  .toString();
...
describe("Trigger", () => {
  ...
  beforeEach(() => {
    ...
    JSONP.mockImplementation(opts => {
      resolveTemplateLoaded();
      opts.success(inviteTemplateHtml);
    });
  });
