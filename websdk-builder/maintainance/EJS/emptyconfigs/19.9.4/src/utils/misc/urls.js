...
const testAgainstSearch = (srch, url) => {
  ...
  url = toLowerCase(url).replace(/^(https?:)?\/\//, "");
  ...
  if (srch.indexOf("re:") === 0) {
    const re = new RegExp(srch.substr(3));
    return re.test(url);
  }
  ...
  if (srch.indexOf("?") > -1) {
    ...
    const urlpartmatches = testUrlPart(srchparts[0], urlparts[0]);
    const querypartmatches = testUrlPart(srchparts[1], urlparts[1]);
  ...
  return testUrlPart(srch, url);
};

function testUrlPart(srch, url) {
