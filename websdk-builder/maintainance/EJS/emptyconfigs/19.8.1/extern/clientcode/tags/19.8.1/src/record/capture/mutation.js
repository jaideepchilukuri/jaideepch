 ...
 * @param {DomTree} tree
 */
var Mutation = function(frm, config, masker, worker, newInfoScanner, ctxId, inputCap, tree) {
  // Assign props
  fs.ext(
    this,
    {
  ...
  if (MutationObserverCtor.name !== "MutationObserver") {
    ...
    var inst = new MutationObserverCtor(function() {});
  ...
  /* pragma:DEBUG_START */
  if (MutationObserverCtor.name !== "MutationObserver") {
    throw new Error("MutationObserver monkey patched!!!");
  }
  /* pragma:DEBUG_END */
