...
utils.Browser = function(usrAgt) {
  ...
  fs.ext(ctx, {
    ...
    isIos: false,
    servUrl:
      (fs.config.deviceDetectionUrl || "https://device.4seeresults.com") + "/detect?accessToken=",
  });
  ...
  var getOsVersion = function(agt, isMob) {
    var vermatch, verb, maj, min, minv, micrv;
      ...
      if (metaViewportTags.length > 0) {
        ...
        for (var i = 0; i < metaViewportTags.length; i++) {
          ...
          if (
            ...
            (user_scalable[1] == "0" || fs.toLowerCase(user_scalable[1]) == "no")
          )
            return false;
