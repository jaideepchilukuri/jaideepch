...
(function (tracker) {
  ...
  var Tracker = function (browser, stg, trackerdata) {
    ...
    this.jrny = new utils.Journey(
      cfg.id || utils.getRootDomain() || 'tracker_customerId',
      utils.APPID.TRIGGER,
      stg.get('rid'),
      browser,
      50
    ...
    this._loadResources(function () {
      ...
      stg.onCommit.subscribe(function () {
  ...
  Tracker.prototype._loadResources = function (cb) {
    ...
    this.queue.enqueue(function (prom) {
      ...
      /* pragma:DEBUG_END */
      utils.loadCSS(cssfilename, function (linkel) {
        /* pragma:DEBUG_START */
  ...
  Tracker.prototype.renderTemplate = function () {
    ...
    // Set up the convert
    if (!this._cvTimeout) {
      this._cvTimeout = setTimeout(function () {
        this._stage = 2;
        this._convertTracker();
      }.bind(this), this.data.cfg.config.trackerConvertsAfter);
    } else if (this._stage == 2) {
      this._convertTracker();
    }

    // Bind to resize
    utils.Bind(window, "tracker:resize", function () {
      // Run the sizing
      this._doSizing();
    }.bind(this));
  ...
  Tracker.prototype.launchSurveyOrQualifier = function () {
    ...
    if (qual && qual.useQualifier) {
      if (isModern) {
        /* pragma:DEBUG_START */
        console.error("tracker: WebSDK's qualifier skipped. The modern tracker lets survey handle the \"qualifier\" feature.");
        /* pragma:DEBUG_END */
        this.goToSurvey();
      } else {
        this.goToQualifier();
  ...
  Tracker.prototype.goToQualifier = function () {
    this.qualifier = new Qualifier(this.br, this.cpps, this.data, this.data.cfg.active_surveydef.qualifier, this.qualhtml, this._displayOpts);

    // Qualified
    this.qualifier.qualified.subscribe(function () {
    ...
    // Disqualified
    this.qualifier.disqualified.subscribe(function () {
  ...
  Tracker.prototype.goToReminder = function () {
    this.reminder = new Reminder(this.br, this.cpps, this.data, this.data.cfg.active_surveydef.reminder, this.rmdrhtml, this._displayOpts);

    this.reminder.accepted.subscribe(function () {
