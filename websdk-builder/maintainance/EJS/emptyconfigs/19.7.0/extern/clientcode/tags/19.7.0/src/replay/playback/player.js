...
(function () {
  ...
  */
   ...
   */
  Player.prototype.loadPageData = function () {
    return new Promise(function (resolve, reject) {
      new utils.AjaxTransport().send({
        ...
        success: function (dtastr) {
          ...
          /* pragma:DEBUG_START */
          console.warn("rp: received data:", this.data);
          /* pragma:DEBUG_END */
  ...
  Player.prototype.videoSyncUp = function (command) {
    ...
    this.signalMessage({
      command: command,
      videoTimeIndex: this.capture.getDuration(),
  ...
  Player.prototype.playNextEvent = function () {
    ...
    if (timeToSkip > 0) {
      ...
      timeToSkip -= this.ui.skipTimeDuration;
      if (timeToSkip < 0) {
        timeToSkip = 0;
      }
    } else {
