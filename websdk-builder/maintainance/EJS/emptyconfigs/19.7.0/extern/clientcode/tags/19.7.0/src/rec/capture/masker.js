...
(function () {
  ...
  Masker.prototype._prepPIIObjectForPage = function (_piiObj, winRef) {
    ...
    if (piiObj.staticVisibleEls) {
      piiObj.selectiveUnMaskZones = fs.ext(piiObj.selectiveUnMaskZones || {}, piiObj.staticVisibleEls);
    ...
    if (piiObj.dynamicVisibleEls) {
      piiObj.selectiveUnMaskZones = fs.ext(piiObj.selectiveUnMaskZones || {}, piiObj.dynamicVisibleEls);
    ...
    if (piiObj.staticWhiteListEls) {
      piiObj.selectiveUnMaskZones = fs.ext(piiObj.selectiveUnMaskZones || {}, piiObj.staticWhiteListEls);
    ...
    if (piiObj.dynamicWhiteListEls) {
      piiObj.selectiveUnMaskZones = fs.ext(piiObj.selectiveUnMaskZones || {}, piiObj.dynamicWhiteListEls);
    ...
    if (piiObj.pagesToSelectiveMask == null) {
      ...
      if (
        upgraded &&
        Object.keys(piiObj.selectiveUnMaskZones).length === 0
      ) {
    ...
    /* pragma:DEBUG_START */
    console.warn("sr: " + (winRef === window.top ? '' : '[frm] '), "************* MASKING SITUATION *************");
    if (useWhiteListing) {
      console.warn("sr: " + (winRef === window.top ? '' : '[frm] '), "whitelisting with " + (piiObj.noRules ? "no rules applied" : piiObj.selectiveUnMaskZones.split(',').length + " rules applied"));
    } else {
      console.warn("sr: " + (winRef === window.top ? '' : '[frm] '), "blacklisting with " + (piiObj.noRules ? "no rules applied" : piiObj.selectiveMaskZones.split(',').length + " rules applied"));
    }
    if (piiObj.visibleInputs && piiObj.visibleInputs.length > 0) {
      console.warn("sr: " + (winRef === window.top ? '' : '[frm] '), "visible input rules: " + piiObj.visibleInputs.split(',').length);
    }
    if (piiObj.redactZones && piiObj.redactZones.length > 0) {
      console.warn("sr: " + (winRef === window.top ? '' : '[frm] '), "redaction rules: " + piiObj.redactZones.split(',').length);
    }
    console.warn("sr: " + (winRef === window.top ? '' : '[frm] '), "*********************************************");
    /* pragma:DEBUG_END */
  ...
  Masker.prototype.removeNodeFromMaskingTargets = function (node) {
    ...
    if (piiObj.useWhiteListing && _elsWhiteList.length > 0) {
      for (i = _elsWhiteList.length - 1; i >= 0; i--) {
        if (node === _elsWhiteList[i] || (node.nodeType === 1 && node.contains(_elsWhiteList[i]))) {
          _elsWhiteList.splice(i, 1);
        }
      }
    } else if (!piiObj.useWhiteListing && _elsBlock.length > 0) {
      for (i = _elsBlock.length - 1; i >= 0; i--) {
        if (node === _elsBlock[i] || (node.nodeType === 1 && node.contains(_elsBlock[i]))) {
    ...
    for (i = _elsInputWhiteList.length - 1; i >= 0; i--) {
      if (node === _elsInputWhiteList[i] || (node.nodeType === 1 && node.contains(_elsInputWhiteList[i]))) {
          ...
          if (elsToDealWith.length > 0) {
            for (i = 0, ellen = elsToDealWith.length; i < ellen; i++) {
              ...
              for (j = _elsWhiteList.length - 1; j >= 0; j--) {
                if ((_elsWhiteList[j].nodeType === 1 && _elsWhiteList[j].contains(tmpEl))) {
        ...
        if (elsToDealWith.length > 0) {
          for (i = 0, ellen = elsToDealWith.length; i < ellen; i++) {
            ...
            for (j = _elsBlock.length - 1; j >= 0; j--) {
              if ((_elsBlock[j].nodeType === 1 && _elsBlock[j].contains(tmpEl))) {
                _elsIgnore.push(tmpEl);
                breakout = true;
                break;
              }
