...
(function (trigger) {
  ...
  var Tracker = function (template, def, cfg, stg, cpps, display, browser) {
    ...
    this.hbi = cfg.config.trackerHeartbeatTimeout;
    ...
    if (fs.config.storage !== utils.storageTypes.MC) {
      //... then check if server errors occurs and save a flag to prevent invites for a while
      this.stg._readyState.subscribe(function () {
        if (stg._serverFails > 0) {
          /* pragma:DEBUG_START */
          console.warn("trigger: brain server is having issues. Invite will be blocked for some time");
          /* pragma:DEBUG_END */
    ...
    this.stg.ready.subscribe(function () {
      ...
      // real one..
      this.stg.set('tracker_hb', this.def.index, this.hbi, false);
      ...
      var onStorageCommit = this.stg.onCommit.subscribe(function () {
        if (stg.get('tracker_hb') === null) {
          if ((utils.now() - this.lastTimeSeenTracker) > this.cfg.config.trackerHeartbeatTimeout) {
            ...
            /* pragma:DEBUG_START */
            console.warn("trigger: The tracker must be closed now, there's no tracker_hb ", stg._data);
            /* pragma:DEBUG_END */
    ...
    this._url = fs.makeURI([
      "$fs.tracker.html?uid=", enc(stg.uid || ''),
      "&sitekey=", enc(fs.config.siteKey),
      "&domain=", enc(utils.getRootDomain()),
      "&gw=", enc(fs.makeURI("trigger/__gwtest__")),
      "&brain_url=", enc(fs.config.brainUrl),
      "&fsrlocale=", enc(cpps.get('locale') || 'en'),
      "&_svu_=", enc(fs.config.surveyUrl),
      "&_cv_=", enc(fs.config.codeVer),
      '&_issh_=', enc(fs.isSelfHosted),
      "&_vt_=", enc(fs.tagVersion),
      "&_au_=", enc(fs.config.analyticsUrl),
      '&_pa_=', enc(fs.assetLocation)
    ].join(""));
    ...
    this.cpps.onSet.subscribe(function (key, value) {
      /* pragma:DEBUG_START */
  ...
  Tracker.prototype.show = function (browser) {
    ...
    this.wref = popup(this._url, 'fsTracker', {
      width: 700,
      height: 450
    }, browser, true, this.cfg.config.centerTrackerPopup);
  };
