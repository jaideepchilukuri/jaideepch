...
(function () {
  ...
  var Qualifier = function (browser, cpps, data, qual, template, displayopts) {
    ...
    if (this.userLocale !== "en") {
      if (fs.isDefined(this.data.cfg.active_surveydef.qualifier.survey.locales[this.userLocale])) {
        this.qual.survey = fs.ext({}, this.data.cfg.active_surveydef.qualifier.survey.locales[this.userLocale]);
      } else {
        /* pragma:DEBUG_START */
        console.warn([
    ...
    this.disqualified.subscribe(function () {
      this.showNoThanks();
    }.bind(this));

    // Bind to validation failed
    this.validationFailed.subscribe(function (msg) {
      /* pragma:DEBUG_START */
  ...
  Qualifier.prototype.render = function () {
    ...
    // Bind to buttons
    utils.Bind(document.getElementById('qualifierForm'), 'qualifier:submit', function (e) {
      utils.preventDefault(e);
      this.validateAndSubmit();
    }.bind(this));

    utils.Bind(document.getElementById('qualCancelButton'), 'qualifier:click', function (e) {
      utils.preventDefault(e);
      this.disqualified.fire();
    }.bind(this));

    utils.Bind(document.getElementById('qualCloseButton'), 'qualifier:click', function (e) {
      utils.preventDefault(e);
      window.close();
    }.bind(this));
  };
