...
fs.provide("rp.Replay.Playback.DomReplay");

fs.require("rp.Replay.Playback.DomTree");
fs.require("rp.Top");

(function () {

  // time to wait for elements to load before producing timeout errors
  var LOADING_TIMEOUT = 120000;
  ...
  DomReplay.prototype.insert = function (index, node) {
    ...
    if (!parent.c || index >= parent.c.length) {
      this.get(parent.id).appendChild(newEl);
    } else {
      this.get(parent.id).insertBefore(newEl, this.get(parent.c[index].id));
    }
  ...
  DomReplay.prototype.updateCharData = function (id, value) {
    ...
    if (!node) {
      console.error("Attempt to change text of unknown node:", id);
      return;
    }
  ...
  DomReplay.prototype.modifyAttr = function (id, key, value) {
    ...
    if (!node) {
      console.error("Attempt to modify attr of unknown node:", id);
      return;
    }
  ...
  DomReplay.prototype.markIframeNotRecorded = function (el) {
    ...
    var promise = this.loadPromises[this.loadPromises.length - 1] ||
      Promise.resolve();
  ...
  DomReplay.prototype.createCanvasPlaceHolder = function (node) {
    ...
    var docStyle = this.root.ownerDocument.styleSheets[this.root.ownerDocument.styleSheets.length - 1];
    if (!docStyle) {
      console.error(
        "No stylesheet found. Canvas",
        node.id,
  ...
  DomReplay.prototype.hasParentSVG = function (node) {
    var parent = this.tree.getById(node.p);
    if (!parent) {
      return false;
    }
    if (node.n.toUpperCase() === "SVG") {
      return true;
    }
    return this.hasParentSVG(parent);
  };
    ...
    this.loadPromises.push(
      new Promise(function (resolve) {
        ...
        var done = function () {
          ...
          el.removeEventListener("error", done, false);
          resolve();
        };

        var timeout = function () {
          console.error("Load timeout on element", el);
          timer = null;
          done();
        };
        ...
        timer = setTimeout(timeout, LOADING_TIMEOUT);
      })
    );
  };
