...
(function () {
  ...
  InputCapture.prototype._watchInput = function (node) {
    ...
    if (tName == "TEXTAREA" || (tName == "INPUT" && __textTypes.indexOf(iptType) > -1)) {
      ...
      this.rec.bind(node, "input", function (e) {
        var targ = e.target || e.srcElement,
          tval = targ.value,
          pval = this._getPreviousValue(targ).toString();
  ...
  InputCapture.prototype._stopWatchingInput = function (node) {
    ...
    if (
      typeof Object.getOwnPropertyDescriptor !== "undefined" &&
      Object.getOwnPropertyDescriptor(node, 'value')
    ) {
      // remove the property descriptor listening to javascript changes
      delete node.value;
    }
  ...
  InputCapture.prototype._listenToJavascriptCausedChanges = function (node) {
    if (
      ...
      // This is so very ugly, I really feel bad, honestly I do, but AFAIK
      // this is the only way.
      var desc = Object.getOwnPropertyDescriptor(Object.getPrototypeOf(node), 'value');
      if (desc) {
        Object.defineProperty(node, 'value', {
          ...
          get: function () {
            ...
            return desc.get.call(node);
          }.bind(this),
          set: function (newValue) {
            var prevValue = this._getPreviousValue(node).toString();
            if (newValue !== prevValue) {
              ...
              this._updateKnownValue(node, newValue);
            }

            // don't recurse infinitely
            return desc.set.call(node, newValue);
          }.bind(this)
        });
      }
    }
  ...
  InputCapture.prototype.untrackInputs = function (node) {
    ...
    for (j = 0; j < known.length; j++) {
      if (node.nodeType === 1 && node.contains(known[j])) {
