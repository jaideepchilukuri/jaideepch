/**
 * @preserve
 * ForeSee Gateway Script v1.3.0. Thursday, October 20th, 2016, 1:39:12 PM
...
(function() {
   ...
   */
  if (typeof(window["_fsDefine"]) != 'undefined') {
    return;
  }
  /**
   ...
   * @returns Boolean
   */
  var acsCmd = function(commandName) {
    var hv = (window.location.hash + '').toLowerCase(),
      cmdToken = 'acscommand';
    commandName = (commandName || '').toLowerCase();
    if (hv.length > 2 && hv.indexOf(cmdToken) > -1 && hv.indexOf(commandName) > -1) {
      return true;
    }
    return false;
  };

  // Then call it once to prevent the minifier from deleting it.
  acsCmd();
  /**
   ...
   */
  (function(global) {
    var req, s, head, baseElement, src,
        ...
        makeRequire: function(relMap, options) {
          ...
          function localRequire(deps, callback, errback) {
            if (deps instanceof Array) {
              eachProp(equivs, function(prp, eql) {
                eachProp(deps, function(drp, dql) {
                  deps[dql] = deps[dql].replace(eql, prp);
                });
              });
            }
    ...
    req = function(deps, callback, errback, optional) {
      ...
      // Do failsafe version check
      if (typeof deps == 'string' && deps.indexOf('.') > -1) {
        if (extractVersionFromFileName(deps) < minCodeVer) {
          return;
        }
      } else if (isArray(deps)) {
        for (var i = 0; i < deps.length; i++) {
          if (deps[i].indexOf('.') > -1 && extractVersionFromFileName(deps[i]) < minCodeVer) {
            return;
          }
        }
      }
    ...
    head = s.head = _d.getElementsByTagName('head')[0];
    ...
    req.createNode = function(config, moduleName, url) {
      var node = config.xhtml ?
        _d.createElementNS('http://www.w3.org/1999/xhtml', 'html:script') :
        _d.createElement('script');
  ...
  _w["_fsRequire"] = _w["_acsRequire"] = require;



  /**
   * Holds the collection of configuration snippets
   * @type {{}}
   */
  var AnswersProductWhitelist = {};



  /**
   * @preserve
   * <#uncomment> ${toObjectProperties(snippets, "AnswersProductWhitelist")} </#uncomment>
   * */

  /**
   * A global value indicating the minimum required code version
   * @type {number}
   */
  var minCodeVer = 1;

  /**
   * Scan all the dependencies and block out of date ones
   * @param deps
   */
  var scanAndBlockDependencies = function(deps) {
    var maxver = 1;
    each(deps, function(p) {
      maxver = Math.max(maxver, extractVersionFromFileName(p));
    });
    minCodeVer = maxver;
    for (var i = 0; i < deps.length; i++) {
      if (extractVersionFromFileName(deps[i]) < maxver) {
        deps.splice(i--, 1);
      }
    }
  };
  ...
  /**
   * Return the version from the file name
   * @param fname
   */
  var extractVersionFromFileName = function(fname) {
    var vExp = /.*\.v([0-9]*)\..*/g,
      ms = vExp.exec(fname);

    return fname.indexOf('.js') == -1 ? minCodeVer : ((ms && ms.length > 1) ? ms[1] : 1);
  };
  /*jshint -W030 */
  /**
  ...
  /**
   * When do dependencies load?
   * @type {string}
   */
  var dependencyTiming = 'domready';
  ...
  var locator = {
    ...
    /**
     * Describes the tag version. This gets modified on the server.
     */
    tagVersionString: !!window['_acsVOverride'] ? window['_acsVOverride'] : '${versionTag}',

    /**
     * Where these files are found
     */
    gatewayLocation: (function() {
      var scrs = getAllScripts(),
        gwScr,
        pgwScr,
        g = "gateway",
        src,
        tm,
        s = '/';
      each(scrs, function(scr) {
        src = attr(scr, "src") || '';
        if (attr(scr, "data-vendor") == "acs" && attr(scr, "data-role") == g) {
      ...
      if (gwScr) {
        ...
        trimDots(src);
        if (tm) {
          dependencyTiming = tm.toLowerCase();
        }
        return src.join(s) + s;
      }
    })()
   ...
   */
  locator.normalizeUrl = function(rooturl, tagver) {
    rooturl = rooturl || '';
    return function(url) {
      // Swap out any replacements
      eachProp(equivs, function(prp, eql) {
        url = url.replace(eql, prp);
      });
      // First, fix the URL
      url = url || '/';
      var addm = 'v=' + encodeURIComponent(locator.tagVersionString);
      if (url.indexOf('//') == -1) {
        url = rooturl.substr(rooturl.length - 1, 1) == '/' && url.substr(0, 1) == '/' ? rooturl + url.substr(1) : rooturl + url;
      }
      return url + ((url.indexOf('?') > -1) ? '&' : '?') + 'v=' + encodeURIComponent(tagver);
    };
  }(locator.gatewayLocation, locator.tagVersionString);

  // Expose the locator to the world
  _w["_acsNormalizeUrl"] = locator.normalizeUrl;
  /**
   * Event Bind
   * @param element
   * @param type
  ...
  /**
   * Will hold the list of dependencies that we will load
   * @type {Array}
   */
  var deps = [];

  /**
   * Holds a list of equivalencies
   * @type {{}}
   */
  var equivs = {};
   ...
   */
  eachProp(AnswersProductWhitelist, function(prd, prop) {
    if (prd["check"] === true || (isFunction(prd["check"]) && prd["check"].call(prd))) {
      var proddeps = prd["dependencies"],
        eqvs = prd["equivalencies"];
      deps = deps.concat(isString(proddeps) ? [proddeps] : proddeps);
      if (eqvs) {
        eachProp(eqvs, function(prp, eql) {
          equivs[eql] = prp;
        });
      }
    }
  ...
  /**
   * Test each equivalency to see if its needed
   */
  eachProp(equivs, function(eqvl, eqsrc) {
    var foundit = false;
    each(deps, function(p, i) {
      if (deps[i] == eqvl) {
        foundit = true;
      }
    });
  ...
  // Remove dependencies that are out of date
  scanAndBlockDependencies(deps);

  /**
   * Normalize all the dependencies
   */
  each(deps, function(p, i) {
    deps[i] = locator.normalizeUrl(p);
  });

  /**
   * Holds the list of callbacks for when both domReady and modules are ready
   * @type {Array}
   */
  var readyList = [],
    acsReadyToken = "acsReady",
    isModulesReady = false,
    readyTimer,
    executeReadies = function() {
      var cbmethod = domReady;
      if (dependencyTiming == 'load') {
        cbmethod = winload;
      }
      cbmethod(function() {
        // Put executions on a little timeout so that modules that done have proper
        // AMD style execution will still be available
    ...
    allReady = window["__" + acsReadyToken + "__"] = function() {
      for (var i = 0; i < arguments.length; i++) {
        if (isFunction(arguments[i])) {
          readyList.push(arguments[i]);
        }
      }
      if (isModulesReady) {
        executeReadies();
  ...
  // Expose acsReady IF the user forgot to do this themselves.
  if (!isDefined(_w[acsReadyToken])) {
    _w[acsReadyToken] = allReady;
  }

  // Make a note of when the gateway script was loaded. We use this later.
  var startTS = !!(_w.performance && _w.performance.timing) ? _w.performance.timing.responseStart : (new Date()).getTime();

  /**
   * Provide some tools for modules that want or need it.
   */
  define("_acs", function() {
    return {
      ...
      "allReady": allReady,
      "domReady": domReady,
      "products": AnswersProductWhitelist,
  ...
  /**
   * Now we have the list of dependencies. Let's load them with AMD.
   */
