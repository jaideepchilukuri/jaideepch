...
(function () {
  ...
  function RecordController() {
    ...
    var storageArg = arguments[2];
      this.jrny = new utils.Journey(
        ...
        utils.getRootDomain() || 'record_customerId',
        recordconfig.clientId || storageArg.get("cp").site || 'record_customerId',
   ...
   * @constructor
   */
  RecordController.prototype.initialize = function (browser, winobj, stg, extrasettings) {
      ...
      stg.ready.subscribe(fs.proxy(function () {
        ...
        browser.ready.subscribe(fs.proxy(function () {
          ...
          if (this.crit.supported()) {
            var stgChecks = this.stg.get(['rt', 'i']);
            ...
            if (!isCrossDomainFrame) {
              ...
              // Log rec event
              if (this.jrny) {
                this.jrny.addEventString(RECLOGGING.RECORDER_STARTED);
              }
   ...
   * @returns {null}
   */
  RecordController.getInstance = function (browser, winobj, stg, extrasettings) {
    if (controllerInstance === null) {
      controllerInstance = new RecordController(browser, winobj, stg, extrasettings);
    }
