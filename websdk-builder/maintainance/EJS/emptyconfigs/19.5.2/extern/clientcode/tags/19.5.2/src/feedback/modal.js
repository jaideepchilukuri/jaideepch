...
(function () {
  ...
  Modal.prototype._showThankyou = function () {
    ...
    var h1s = this.$content.$("h1"),
      cbut = this.$content.$(".acs-close-button")[0],
      focusModal = document.getElementById("acsModalContent");
    for (var p = 0; p < h1s.length; p++) {
  ...
  Modal.prototype.add = function (win) {
    ...
    var head = this.head = $("<div class=\"acsModalContent\" id=\"acsModalContent\"></div>"),
      mdlhead = $("<div class=\"acsModalHead\"></div>"),
      closebtn = $("<img id=\"fsrModalFocus\" src=\"" + fs.makeURI("$templates/feedback/" + (this.instcfg.template || 'default') + '/closeBtn.svg') + "\" class=\"acsModalCloseButton\">");
    closebtn.setAttribute("tabindex", "0");
    closebtn.setAttribute("alt", "Close Button");
    mdlhead.setAttribute("title", "ForeSee Survey");
    ...
    if (!this.cfg.preview) {
      ...
      utils.Bind(closebtn, "feedback:keypress", fs.proxy(function (e) {
        // Signal the event to the eventing server
  ...
  Modal.prototype.focusOnBadge = function () {
    var badgeEl = this.instcfg.badge.$el;
