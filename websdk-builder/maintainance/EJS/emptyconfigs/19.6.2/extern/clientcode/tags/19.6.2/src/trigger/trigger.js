  ...
  Trigger.prototype.popTracker = function (invite) {
    ...
    if (!this.didPopTrackerAlready) {
      ...
      var finishTrackerSetup = function () {
        ctx.tracker = new Tracker(invite.template, ctx.surveydef, config, utils.getBrainStorage(ctx.browser, ctx.stg.uid), ctx.cpps, invite.display, ctx.browser);
      ...
      if (invite) {
        ...
        var ist = getInviteSetupInstance(this, ctx.browser, ctx.stg, ctx.cpps, false, ctx.jrny, function () {
          /* pragma:DEBUG_START */
          console.log("trigger: finished after-the-fact invitation setup");
          /* pragma:DEBUG_END */
          ctx.tracker = new Tracker(this.invite.template, ctx.surveydef, config, utils.getBrainStorage(ctx.browser, ctx.stg.uid), ctx.cpps, this.invite.display, ctx.browser);
