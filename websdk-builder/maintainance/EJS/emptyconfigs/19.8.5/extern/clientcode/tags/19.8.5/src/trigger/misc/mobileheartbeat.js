...
var MobileHeartbeat = function(invitetype, cfg, def, cpps, rid, locale) {
  ...
  this._measureName =
    this.def.name +
    "-" +
    (fs.isDefined(this.def.site) ? this.def.site + "-" : "") +
    (fs.isDefined(this.def.section) ? this.def.section + "-" : "") +
...
MobileHeartbeat.prototype.init = function(usercontactinfo, callback) {
  ...
  var params = {
    ...
    cpps:
      "version=" + encodeURIComponent(this.cfg.config.version) + "&" + this.cpps.toQueryString(),
  };
...
MobileHeartbeat.prototype.beginHeartbeat = function() {
  ...
  console.log(
    ...
    config.config.onExitMobileHeartbeatInterval
  );
  /* pragma:DEBUG_END */
  var hb = function() {
    Services.ping(
