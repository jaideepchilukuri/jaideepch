  ...
  nextTick(function() {
    ...
    var finalSetup = function() {
      ...
      for (i = 0; i < productNames.length; i++) {
        dependencies = dependencies.concat(productConfig[productNames[i]].dependencies);
      }

      if (!dm) {
        if (globalConfig.modernRecord && globalConfig.codeVer.indexOf("19.7.") === 0) {
        ...
        _fsRequire(dependencies, function() {
          if (!_W["__" + fsrd + "__"]) {
            ...
            _W["__" + fsrd + "__"] = _W["__acsReady__"] = _W["fsReady"] = _W[
              "acsReady"
            ] = function() {
              var args = arguments;
              nextTick(function() {
      ...
      _fsRequire([locator.normalizeUrl("$fs.utils.js")], function(utils) {
        ...
        var finishSelfHost = function(setts) {
          ...
          eachProp(setts, function(obj, prop) {
            if (
              prop != "global" &&
              !(isDefined(globalConfig.products[prop]) && globalConfig.products[prop] === false)
            ) {
              if (
                prop === "record" &&
                globalConfig.modernRecord &&
                globalConfig.codeVer.indexOf("19.7.") === 0
              ) {
...
/**
 * Walk through the productConfigs and evaluate their check functions,
 * returning a list of products which have configs.
 */
function materializeConfigs() {
  ...
  eachProp(productConfig, function(obj, prop) {
    if (
      ...
      globalConfig.products[prop.toLowerCase()] === false
    ) {
      obj.check = false;
    }
