...
fs.winReady(function() {
  ...
  function popWindowNormal() {
    ...
    function popWindowWithConfig(globalConfig, mId, cpps, cid) {
      ...
      config.global = globalConfig.global;

      // Preview does not have to communicate with the backend so it does not need a Journey
      if (siteKey !== "preview") {
        var appId = utils.APPID.FEEDBACK;

        journey = new utils.Journey({
          customerId: cid,
          appId: appId,
          stg: utils.getBrainStorage(browser, userId, fs.config.siteKey),
          browser: browser,
          useSessionId: false,
          usePopupId: true,
        });
        journey.config = globalConfig.global;
      }
