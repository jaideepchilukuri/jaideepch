...
(function () {
  ...
  FullPageSurvey.prototype.onNetworkError = function (obj) {
    ...
    utils.Bind(this.$closebtn, "feedback:click", function (event) {
    ...
    utils.Bind(okBtn, "feedback:click", function (event) {
      this.hide(event, true);
    }.bind(this));
   ...
   */
  FullPageSurvey.prototype.hide = function (event, noBtns) {
    window.document.body.innerHTML = '';
    ...
    if (this.jrny) {
      if (event && event.target && !event.target.classList.contains("acs-feedback__button--epilogue")) {
        this.jrny.addEventObj({
          "name": 'feedback_abandoned',
          "properties": {
            "mid": [this.cfg.mid]
  ...
  FullPageSurvey.prototype.remove = function (noBtns) {
