...
(function () {
  ...
  var Modal = function (survey, browser, instconfig, errortemplate, modaltemplate, eptemplate) {
    ...
    this.networkError.subscribe(
      fs.proxy(function (obj) {
        ...
        if (!this.cfg.preview) {
          this.positionModal();
          utils.Bind(cbut, "feedbackModal:click", fs.proxy(function () {
            this.remove();
          }, this));
        }
  ...
  Modal.prototype.add = function (win) {
    ...
    if (!this.cfg.preview) {
      var ctx = this;
      utils.Bind(backFace, "feedbackModal:click", fs.proxy(function (e) {
    ...
    if (!this.cfg.preview) {
      utils.Bind(closebtn, "feedbackModal:click", fs.proxy(function (e) {
        // Signal the event to the eventing server
        if (this.jrny) {
          this.jrny.addEventObj({
            "name": "feedback_abandoned",
      ...
      utils.Bind(closebtn, "feedbackModal:keypress", fs.proxy(function (e) {
        // Signal the event to the eventing server
  ...
  Modal.prototype._trapKeyBoard = function (modalContainer, firstFocusEl) {
    ...
    utils.Bind(document.body, 'feedbackModal:focus', function (e) {
      ...
      var elInvite = document.querySelector("#acsMainInvite");
      if (elInvite && elInvite.contains(target)) {
        return;
      }
