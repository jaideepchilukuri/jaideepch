...
(function () {
  ...
  var Logger = function (stg, browser, recorder, config) {
    ...
    // The cors object
    this.cors = new utils.CORS(browser, stg.fstg);
    ...
    if (!this.isIframeMode) {
      ...
      this.TransportReady.subscribe(fs.proxy(function () {
        /* pragma:DEBUG_START */
      ...
      this.recorder.ready.subscribe(fs.proxy(function () {
         ...
         */
        this.recorder.EmergencySendRequired.subscribe(fs.proxy(function () {
      ...
      this.iFrameCommitItv = setInterval(fs.proxy(this._flushAndTransmit, this), 1000);
    }
  ...
  Logger.prototype._syncSession = function (gsessionid, transmitting, canceled) {
    ...
    var storage = this._storage || this._backup_storage;
    ...
    if (this.refreshLastTime) {
      ...
      if (utils.now() - this.refreshLastTime > expireDuration) {
        ...
        this.stg.erase([SESSION_SYMBOLS.GLOBALSESSIONID, SESSION_SYMBOLS.SESSIONID]);
      } else {
        /* pragma:DEBUG_START */
    ...
    if (!storage.get(SESSION_SYMBOLS.SESSION)) {
      this._session = {};
      ...
      this.stg.set(SESSION_SYMBOLS.GLOBALSESSIONID, this.gsessionid);
      ...
      storage.set(SESSION_SYMBOLS.SESSION, this._session);

      // Save it real quick
      storage.commit();
    } else {
      // Get or create the storage array
      this._session = storage.get(SESSION_SYMBOLS.SESSION);

      this.sessionid = this._session[SESSION_SYMBOLS.SESSIONID];
      this.gsessionid = this._session[SESSION_SYMBOLS.GLOBALSESSIONID];

      this.stg.set(SESSION_SYMBOLS.SESSIONID, this.sessionid);
  ...
  Logger.prototype.processImmediately = function (delay) {
    utils.Healthy(this.browser, ["cxreplay"], fs.proxy(function () {
      // Perform the send
      this.cors.send({
        method: 'GET',
  ...
  Logger.prototype.startHeartbeat = function () {
    if (!this.heartbeatTransmit) {
      this._ping();
      this.heartbeatTransmit = setInterval(fs.proxy(function () {
        this._ping();
      }, this), 45000);
    }
  };
  ...
  Logger.prototype.startStateMonitor = function () {
    if (!this.stateMonitor) {
      this._monitorState();
      this.stateMonitor = setInterval(fs.proxy(function () {
        this._monitorState();
      }, this), 2000);
    }
  };
  ...
  Logger.prototype.startStorageMonitor = function () {
    if (!this.storageMonitor) {
      this.storageMonitor = setInterval(fs.proxy(function () {
        this.log(this.recorder, Logger.EVENT_TYPES.HEARTBEAT, {
          'ofs': (new Date()).getTimezoneOffset()
        }, -1);
        this._flushAndTransmit();
      }, this), (this.browser.isMobile) ? 10000 : 20000);
    }
  };
