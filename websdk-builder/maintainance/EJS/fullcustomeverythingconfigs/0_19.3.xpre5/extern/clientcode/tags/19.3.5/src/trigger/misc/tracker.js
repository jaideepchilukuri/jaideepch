...
(function (trigger) {
  ...
  var Tracker = function (template, def, cfg, stg, cpps, display, browser) {
    ...
    var hbf = fs.proxy(function (notemerg) {
      this.stg.set('page_hb', utils.now(), cfg.config.trackerHeartbeatTimeout, !notemerg, utils.persistDataType.TRACKER);
    }, this);
    this._heartbeat = setInterval(hbf, Math.round(cfg.config.trackerHeartbeatTimeout / 2));
    // Call it right away
    hbf(true);
