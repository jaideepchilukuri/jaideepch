  ...
  FcpClient.promptForFCPCredentials({}, function(rs) {
    ...
    fcp.listClients(function(success, data) {
      if (!success) {
        ...
        console.log(chalk.yellow(`Complete client list (${chalk.magenta(data.length.toString())} results):`));
  ...
  FcpClient.promptForFCPCredentials({}, function(rs) {
    ...
    fcp.listSitesForClient(clientProperties.client.clientid, function(success, data) {
      if (!success) {
        ...
        console.log(chalk.yellow(`Complete sitekey list (${chalk.magenta(data.length.toString())} results):`));
        for (let i = 0; i < data.length; i++) {
          const site = data[i];
          if (i > 0) {
            console.log(chalk.grey("---------------------------------------------"));
  ...
  fsGulpUtil.promptForValuesIfNeeded(
    ...
    function(res) {
      if (res.client_search_term && res.client_search_term.length > 0) {
        ...
        console.log(
          chalk.magenta("Searching for clients with the term: ", '"', chalk.yellow(res.client_search_term), '"', "..")
        );
        FcpClient.promptForFCPCredentials({}, function(rs) {
          /**
           * Set up an instance of the FCP client
          ...
          fcp.lookupClient(res.client_search_term, function(success, data) {
            if (!success) {
              ...
              console.log(
                chalk.yellow("Client results (", chalk.magenta(data.clients.length.toString()), " results):")
              );
              ...
              console.log(
                chalk.yellow("\n\nSite results (", chalk.magenta(data.sites.length.toString()), " results):")
              );
  ...
  fsGulpUtil.promptForValuesIfNeeded(
    ...
    function(res) {
      if (res.client_name && res.client_name.length > 0 && res.metadata && res.metadata.length > 0) {
        ...
        console.log(
          `${chalk.magenta("Making a client with the name: ")}"${res.client_name.yellow}"${chalk.magenta("..")}`
        );
        FcpClient.promptForFCPCredentials({}, function(rs) {
          /**
           * Set up an instance of the FCP client
           ...
           */
          const fcp = new FcpClient(rs.username, rs.password, rs.environment);
          fcp.makeClient(res.client_id, res.client_name, res.metadata, `Created client ${res.client_name}`, function(
            success,
            client
          ) {
            if (!success) {
...
function createSite(cb) {
  FcpClient.promptForFCPCredentials(
    ...
    function(rs) {
      console.log(
        chalk.magenta("Creating a site for client id ") + chalk.yellow(rs.clientId.toString()) + chalk.magenta(".")
      );
      /**
       * Set up an instance of the FCP client
       * @type {FCPClient}
              ...
              if (res.sitekey && res.sitekey.length > 0) {
                ...
                fcp.doesSiteKeyExist(res.sitekey, function(success, exists, clientinfo) {
                  if (!success) {
                    ...
                    console.log(
                      `${chalk.magenta("Making a site key with the name: ")}"${chalk.yellow(res.sitekey)}${
                        '"..'.magenta
                      }`
                    );
                    fcp.makeSite(res.sitekey, rs.clientId, res.alias, `Created site key ${res.sitekey}`, function(
                      success,
                      site
                    ) {
                      if (!success) {
                        ...
                        console.log(chalk.magenta("Site "), site.name, chalk.magenta("was created."));
                        if (cb) {
                          return cb();
                        }
                      }
...
function pushCode(dist, version, dev, testEnvs, isProd, rs, cb) {
  ...
  console.log(chalk.magenta("Packaging code version for FCP: "), chalk.yellow(version.toString()), chalk.magenta("."));
      ...
      optionallyPromptForCredentials(
        ...
        function(rs) {
          function postCodeToEnv(env, cb) {
            const envShort = Object.keys(FcpClient.environments).find(key => FcpClient.environments[key] === env);
...
function pushDefaultConfig(version, getGlobalConfig, rsIn, cb) {
  optionallyPromptForCredentials(rsIn, {}, function(rs) {
     ...
     */
    const fcp = new FcpClient(rs.username, rs.password, rs.environment);
    fcp.postDefaultConfig(JSON.stringify(defCfg), `Posted a new default configuration (${version})`, function(
      success,
      result
    ) {
      if (!success) {
    ...
    function(rs) {
      ...
      fcp.getClient(clientProperties.client.clientid, function(success, client) {
        if (!success) {
          ...
          fcp.makeContainer(clientProperties.client.sitekey, environment, client.id, rs.notes, function(
            success,
            message
          ) {
            if (!success && message !== "Container name already exists") {
              ...
              console.log(
                chalk.magenta(`Pushing code to ${environment} for client ${clientProperties.client.clientid}:`),
              ...
              for (const product of configs.products) {
                if (globalConfig.products[product] || (product === "fbmods" && globalConfig.products.feedback)) {
...
function promoteProd(client) {
  FcpClient.promptForFCPCredentials(
    ...
    function(rs) {
      const fcp = new FcpClient(rs.username, rs.password, rs.environment);
      fcp.promoteStgToProd(client.sitekey, rs.notes, ["trigger", "record"], function(success, result) {
        console.log(result);
      });
    }
  );
  ...
  function updateContainerConfig(fcp, containerObj, siteKey, resolveContainer, rejectContainer) {
    ...
    function gotContainer(success, container) {
      ...
      if (container.config_tag) {
        ...
        fcp.getConfigForSiteContainer(siteKey, containerObj.containerName, containerObj.config_tag, function setConfig(
          data
        ) {
