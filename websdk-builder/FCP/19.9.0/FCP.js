 ...
 * Post code to FCP
 */
function pushCode(pjson, clientProperties, testEnvs, isProd, cb) {
  ...
  const distloc = `${pjson.build.dist}/code/${pjson.version}`;
      ...
      FcpClient.promptForFCPCredentials(
        ...
        function(rs) {
          function postCodeToEnv(env, cb) {
            ...
            console.log(`Pushing code to: ${envShort}`);

            const fcp = new FcpClient(rs.username, rs.password, env);
            fcp.postCodeVersion(buffer, rs.notes, pjson.version, rs.latest, function(
              success,
              data
            ) {
              if (!success) {
...
const BuildAndPushToEnv = function(pjson, clientProperties, environment, cb = function noop() {}) {
  ...
  FcpClient.promptForFCPCredentials(
    ...
    function(rs) {
      ...
      const triggerConfigJSON = configs.trigger;
      const recordConfigJSON = configs.record;
      const feedbackConfigJSON = configs.feedback;

      const triggerGWConfig = configs.gateway.trigger;
      const recordGWConfig = configs.gateway.record;
      const feedbackGWConfig = configs.gateway.feedback;
            ...
            function(success, message) {
              if (!success && message !== "Container name already exists") {
                ...
                if (globalConfig.products.feedback) {
                  if (publishFeedbackConfig) {
                    console.log(
                      chalk.grey("  * "),
                      chalk.red("feedback (this is normally done in CXSuite)")
                    );
                    nbProductToUpload++;
                  } else {
                    console.log(
                ...
                const pushProduct = (name, config, GWConfig) => {
                ...
                if (globalConfig.products.trigger)
                  pushProduct("trigger", triggerConfigJSON, triggerGWConfig);

                if (globalConfig.products.record)
                  pushProduct("record", recordConfigJSON, recordGWConfig);

                if (publishFeedbackConfig && globalConfig.products.feedback)
                  pushProduct("feedback", feedbackConfigJSON, feedbackGWConfig);
              }
            }
          );
        }
    ...
    fsGulpUtil.promptForValuesIfNeeded(
      ...
      function(res) {
        if (res.sendCodeEmail == 1) {
          const recipient = `${creds.username.substr(0, creds.username.indexOf("@"))}@foresee.com`;
