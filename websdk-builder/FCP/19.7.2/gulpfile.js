  ...
  return new Promise((resolve, reject) => {
    ...
    exec(_cmd.join(""), (err, stdout, stderr) => {
      if (err) {
        ...
        return reject("Timeout/connection problem");
      }
      if (!stdout.length) {
        reject("Failed to download file: " + stderr);
        try {
          fs.unlinkSync(fname);
        } catch (e) { }
        return;
      ...
      var statusCode = parseInt(status[0]);
      var contentType = status[1];
      if (status.length !== 2) {
        reject("Bad stdout: " + stdout);
        try {
          fs.unlinkSync(fname);
        } catch (e) { }
        return;
  ...
  for (let i = 0; i < unrchables.length; i++) {
    ...
    for (let j = 0; j < commonBadUrlPrefixes.length; j++) {
      ...
      if (unchr.url.startsWith(prefix)) {
        console.log(
          `[${bucket}: ${i + 1} / ${unrchables.length}] SKIPPED [${prefix}] `.yellow,
          unchr.url,
          "for".yellow,
          srvr
        );
        let res = await delFileAsync(srvr, unchr);
        skip = true;
        break;
      }
    }
    ...
    try {
      ...
      try {
        ...
        console.log(
          `[${bucket}: ${i + 1} / ${unrchables.length}] FAILED [${e}] `.red,
          unchr.url,
          "for".yellow,
          srvr
        );
    ...
    if (
      ...
      console.log(
        `[${bucket}: ${i + 1} / ${unrchables.length}] FAILED [got HTML] `.red,
        unchr.url,
        "for".yellow,
        srvr
      );
    ...
    console.log(
      `[${bucket}: ${i + 1} / ${unrchables.length}] SUCCESS [${fileres.statusCode}] ${
        fileres.contentType
        } (${fileres.data.length} bytes)`.green,
      unchr.url,
      "for".yellow,
      srvr
    );
    ...
    unchr.file = fileres;
    let res = await postFileAsync(srvr, unchr);
  }
};
...
gulp.task("loop_unreachables", function (cb) {
  function runit(next) {
    runSequence(["unreachables"], next);
  }
  function wait(next) {
    return function () {
      setTimeout(function () {
