...
try {
  ...
  if (taskName === 'test_debug' || taskName === 'test_prod') {
    ...
    var codeVersionOverride = process.env['CC_CODE_VERSION'];
    var gatewayVersionOverride = process.env['CC_GATEWAY_VERSION'];
    var devPortOverride = process.env['CC_DEV_PORT'];
    var sslDevPortOverride = process.env['CC_SSL_DEV_PORT'];
    var configDirOverride = process.env['CC_CONFIG_DIR'];

    if (codeVersionOverride) {
      console.log('Overriding code_version with '.red + codeVersionOverride.yellow + ' because CC_CODE_VERSION is set'.red);
      pjson.code_version = codeVersionOverride;
    }

    if (gatewayVersionOverride) {
      console.log('Overriding gateway_version with '.red + gatewayVersionOverride.yellow + ' because CC_GATEWAY_VERSION is set'.red);
      pjson.gateway_version = gatewayVersionOverride;
    }

    if (devPortOverride) {
      console.log('Overriding dev server port with '.red + devPortOverride.yellow + ' because CC_DEV_PORT is set'.red);
      pjson.build.ports[0] = parseInt(devPortOverride, 10);
    }

    if (sslDevPortOverride) {
      if (sslDevPortOverride === "false") {
        console.log('Disabling SSL dev server because because CC_SSL_DEV_PORT=false'.red);
        isSSL = false;
      } else {
        console.log('Overriding SSL dev server port with '.red + sslDevPortOverride.yellow + ' because CC_SSL_DEV_PORT is set'.red);
        pjson.build.ports[1] = parseInt(sslDevPortOverride, 10);
      }
    }

    if (configDirOverride) {
      console.log('Overriding config dir with '.red + configDirOverride.yellow + ' because CC_CONFIG_DIR is set'.red);
      pjson.build.config = configDirOverride;
      clientProperties = require(configDirOverride + '/client_properties');
    }
  }
} catch (e) {}
...
var buildJSProject = function (codever, distloc, proj, projInfo, topdeps, writefile, cb) {
  ...
  gulp.src([externprefix + '/clientcode/tags/' + codever + '/src/' + proj + '/**/*.js'])
    ...
    .pipe(gulpif(isProd, uglify(uglifySetts).on('error', function (err) {
...
gulp.task('_templates', function (cb) {
  ...
  for (var p = 0; p < templateproducts.length; p++) {
    ...
    for (i = 0; i < dirs.length; i++) {
      ...
      gulp.src([
          srcpath + dirs[i] + '/sass/main.scss',
          srcpath + dirs[i] + '/sass/light.scss'
        ])
...
gulp.task('_gateway', function (cb) {
  let paths = {
    dest: pjson.build.extern + '/gateway/tags/' + pjson.gateway_version + '/dist',
    gatewaysrc: pjson.build.extern + '/gateway/tags/' + pjson.gateway_version + '/src/gateway/**/*.js',
    configsrc: pjson.build.extern + '/gateway/tags/' + pjson.gateway_version + '/src/config/**/*.js',
    test: pjson.build.extern + '/gateway/tags/' + pjson.gateway_version + '/testharness/',
    legal: pjson.build.extern + '/gateway/tags/' + pjson.gateway_version + '/bin/legalheader.txt'
  };
  ...
  if (!fs.existsSync(path.join(paths.dest, "gateway" + (isProd ? ".min" : "") + ".js"))) {
    ...
    console.log(
      "Building gateway ".grey + pjson.gateway_version.toString().yellow +
      " with a default code version of ".grey + defaultCodeVer.toString().yellow +
      ".".grey
    );
    ...
    console.log("Building gateway not necessary.".yellow);
    process.nextTick(cb);
  }
});
...
gulp.task('sdk', [
  ...
  runSequence(['_gateway', '_templates', '_customtemplates', '_images', '_html', '_clientassets'], function () {
 ...
 */
gulp.task('test_debug', function (cb) {
  rimraf('./dist', function () {
    runSequence(['sdk', '_clientassets'], function () {
      // Set an instance UID
      ...
      var textfilehandler = function (filename, contents, headers) {
        ...
        if (filename.indexOf('gateway.js') > -1 || filename.indexOf('gateway.min.js') > -1 || filename.indexOf('gatewayconfig.js') > -1 || filename.indexOf('gatewayconfig.min.js') > -1) {
          ...
          var feedbackProdConfig = fs.readFileSync(pjson.build.config + '/productconfig/feedback/product_config.js', 'utf8').toString(),
      ...
      var serverDirs = ['./dist', './smoketest', pjson.build.extern + '/gateway/tags/' + pjson.gateway_version + '/dist'];
            ...
            fcp.doesSiteKeyExist(res.sitekey, function (success, exists, clientinfo) {
              if (!success) {
                ...
                if (exists) {
                  ...
                  fcp.makeSite(res.sitekey, rs.clientId, res.alias, 'Created site key ' + res.sitekey, function (success, site) {
                    if (!success) {
 ...
 * List publishers
 */
gulp.task('list_publishers', function (cb) {
  console.log("Listing publishers...".magenta);
  FcpClient.promptForFCPCredentials({}, function (rs) {
    fsGulpUtil.promptForValuesIfNeeded({
      site_name: null
    }, function (userInput) {
      ...
      fcp.getPublishersForSitekey(siteKey, function (success, publishers) {
        if (success) {
          if (!publishers || publishers.length < 0) {
            ...
            console.log("There is no publisher for site", siteKey.magenta);
          } else {
            console.log("Complete publishers list (".yellow + publishers.length.toString().magenta + " results):".yellow);
            for (var i = 0; i < publishers.length; i++) {
              var publisher = publishers[i];
              if (i > 0) {
                console.log("---------------------------------------------".grey);
              }
              console.log("  Publisher:".magenta, "[ID:".grey, publisher.id.toString().yellow + "]".grey, publisher.user);
              console.log(" permission:".magenta, publisher.permission);
            }
          ...
          console.error(
            "List publishers for site", siteKey.magenta,
            "Error: ", publishers.red
          );
          if (cb) cb();
        }
      });
 ...
 * Remove publishers
 */
gulp.task('remove_publisher', function (cb) {
  console.log("Removing publisher...".magenta);
  FcpClient.promptForFCPCredentials({}, function (rs) {
    fsGulpUtil.promptForValuesIfNeeded({
      site_name: null,
      publisher_id: null
      ...
      fcp.removePublisherForSitekey(siteKey, publisherId, function (success, data) {
        if (success) {
          console.log('Publisher [', publisherId.yellow, '] has been successfully removed from site ', siteKey.magenta);
        } else {
          console.error(
            'Failed to remove publisher [', publisherId.yellow, '] for site', siteKey.magenta,
            '\n Error:', data.red
          );
        }
        if (cb) cb();
      ...
      FcpClient.promptForFCPCredentials({
        ...
        var completePost = function (env) {
          ...
          fcp.postCodeVersion(buffer, rs.notes, pjson.version, rs.latest, function (success, data) {
            var envShort = Object
              .keys(FcpClient.environments)
              .find(key => {
                return FcpClient.environments[key] === env
              });

            if (!success) {
              console.log('𐄂'.red, envShort + ":", data.toString().red);
            } else {
              console.log('✓'.green, envShort + ":", data.toString().magenta);
            }
          });
        };
        if (testEnvs) {
...
gulp.task('push_default_config', ['_pullextern'], function (cb) {
  FcpClient.promptForFCPCredentials({}, function (rs) {
    ...
    if (rs.environment == FcpClient.environments.dev) {
      ...
      console.log("FCP environment:".magenta, "STG".yellow);
...
gulp.task('get_self_hosted', ['sdk'], function (cb) {
  FcpClient.promptForFCPCredentials({}, function (rs) {
    var gatewayFile = fs
      .readFileSync(pjson.build.extern + '/gateway/tags/' + pjson.gateway_version + '/dist/gateway.min.js')
      .toString();
    var sampleHTMLFile = fs
      .readFileSync('./bin/sampleselfhost_page.html')
      .toString();
    var embedSnippet = fs
      .readFileSync('./bin/embedsnippet.html')
      .toString();

    gatewayFile = gatewayFile
      .toString()
      .replace(/\/\*\*[^@\/]*@preserve[^\[\/]*\[GENERAL_CONFIG\][^\/]*\//gi, "globalConfig = " + JSON.stringify({
        selfHosted: true,
        codeVer: pjson.code_version,
        siteKey: clientProperties.client.sitekey,
        gateway: pjson.fcp,
 ...
 */
gulp.task('update-configs', ['_pullextern'], function (cb) {
  ...
  function listSitesForClient(fcp, clientObj, resolveClient, rejectClient) {
    const clientInfo = 'Client Name: "' + clientObj.clientName + '" Client ID:  "' + clientObj.clientId + '" ';
    fcp.listSitesForClient(clientObj.clientId, function (success, sites) {
      if (!success) {
        ...
        if (containers.length) {
          ...
          let containerPromises = containers.map(function (container) {
            return new Promise(function (resolveContainer, rejectContainer) {
              ...
              updateContainerConfig(fcp, containerObj, siteObj.siteKey, resolveContainer, rejectContainer);
  ...
  function updateContainerConfig(fcp, containerObj, siteKey, resolveContainer, rejectContainer) {
    ...
    function gotContainer(success, container) {
      if (!success) {
        return reject('getContainerForSitekey Error: for siteKey "' + siteKey + '" container "' + containerObj.containerName + '": ' + container);
      }

      if (container.config_tag) {
        containerObj.config_tag = container.config_tag;

        fcp.getConfigForSiteContainer(siteKey, containerObj.containerName, containerObj.config_tag, setConfig);
        ...
        function setConfig(data) {
          ...
          function done(success, data) {
            return success ? resolveContainer() : rejectContainer('postDefaultConfigForSiteContainer error: ' + data.message);
          }
...
const getAsync = function (url) {
  return new Promise((resolve, reject) => {
    ...
    let _cmd = ["curl '", url, "' -H 'upgrade-insecure-requests: 1' -H 'user-agent: Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/69.0.3489.0 Safari/537.36' -H 'accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8' -H 'accept-encoding: gzip, deflate, br' -H 'accept-language: en-US,en;q=0.9,fr;q=0.8' -m 10 -s -o /dev/null -w '%{http_code}' --compressed"];
    var fname = uuidV4().toString().replace(/-/gi, '').substr(0, 10);
    exec(_cmd.join(''), (err, stdout, stderr) => {
      ...
      if (stdout.length > 0) {
        ...
        if (statusCode == 200) {
          let _cmd = ["curl '", url, "' -H 'upgrade-insecure-requests: 1' -H 'user-agent: Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/69.0.3489.0 Safari/537.36' -H 'accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8' -H 'accept-encoding: gzip, deflate, br' -H 'accept-language: en-US,en;q=0.9,fr;q=0.8' -m 10 -s -o /dev/null -w '%{content_type}' --compressed"];
          exec(_cmd.join(''), (err, stdout, stderr) => {
            if (err) {
              // node couldn't execute the command
              return reject(err);
            ...
            if (stdout.length > 0 && stdout.indexOf('/') > -1) {
              var contentType = stdout;

              let _cmd = ["curl '", url, "' -H 'upgrade-insecure-requests: 1' -H 'user-agent: Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/69.0.3489.0 Safari/537.36' -H 'accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8' -H 'accept-encoding: gzip, deflate, br' -H 'accept-language: en-US,en;q=0.9,fr;q=0.8' -m 10 -s -o " + fname + ".dat --compressed"];
              exec(_cmd.join(''), (err, stdout, stderr) => {
                if (err) {
                  // node couldn't execute the command
                  return reject(err);
                }
                var data = fs.readFileSync(fname + '.dat');
                fs.unlinkSync(fname + ".dat");
                resolve({
                  contentType,
                  statusCode,
                  data
...
const handleUnreachablesAsync = async function (srvr) {
  ...
  let unrchables;
  try {
    unrchables = await getAsync(srvr + "/services/unreachables");
  } catch (e) {
    console.log("Could not reach ", srvr);
    return;
  }
  ...
  for (let i = 0; i < unrchables.length; i++) {
    let unchr = unrchables[i];
    console.log("[" + (i + 1) + "/" + unrchables.length + "] Getting".yellow, unchr.url, "for".yellow, srvr);

    let fileres;
    try {
      fileres = await getAsync(unchr.url);
    } catch (e) {
      console.log("Coult not get ".red, unchr.url);
      failures.push(unchr);
      continue;
    }
    fileres.data = await compressAsync(fileres.data);
    console.log(`GOT [${fileres.statusCode}] ${fileres.contentType} (${fileres.data.length} bytes)`);
    fileres.data = fileres.data.toString('base64');
    unchr.file = fileres;
    successes.push(unchr);
  }
  console.log(`Posting successes (${successes.length})...`);
  for (let i = 0; i < successes.length; i++) {
    let res = await postFileAsync(srvr, successes[i]);
    console.log("Did ", i + 1, "of", successes.length);
  }
  console.log(`Posting failures (${failures.length})...`);
  for (let i = 0; i < failures.length; i++) {
    let res = await delFileAsync(srvr, failures[i]);
    console.log("Did ", i + 1, "of", failures.length);
  }
 ...
 */
gulp.task('unreachables', function (cb) {
...
* Push the DEFAULT config
*/
gulp.task('push_config', ['_pullextern'], function (cb) {
  ...
  fsGulpUtil.promptForValuesIfNeeded(container, function (res) {
    if(res) {
      ...
      FcpClient.promptForFCPCredentials({
         ...
         */
        var fcp = new FcpClient(rs.username, rs.password, rs.environment);
        fcp.makeContainer(clientProperties.client.sitekey, container, clientProperties.client.clientid, rs.notes, function (success, message) {
          if (!success && message !== "Container name already exists") {
            console.log("Failed to create container: ".red + message.red, rs.environment);
          } else {
            fcp.postDefaultConfigForSiteContainer(clientProperties.client.sitekey, container, JSON.stringify(defCfg), "Posted a new default configuration (" + pjson.version + ")", function (success, result) {
              if (!success) {
                console.log("Was not able to connect. Message: ".red, result);
              } else {
                console.log("Success: ".magenta, result);
 ...
 * Push Product Config to FCP
 */
gulp.task('push_products', function (cb) {
  var container = { containerName: null },
  emailOpts = /^[01]$/.test(envVariableOptions.email) ? {} : {
    sendCodeEmail: null
  };
