...
import TablePV from "../components/table";
...
class MethodInjector extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      guid: false,
  ...
  render() {
    return (
      <div>
        <Subheader>
          Choose a method you want to run test
        </Subheader>
        <div id="methInjection" style={buttonContainer}>
          {this._renderMenuButtons(injectButtons, this._clickMethod)}
        </div>

        <Subheader style={this.state.activePV ? updateStyles : {}}>
          List of the Active PVS
        </Subheader>
        <div>
          <TablePV activePV={this.state.activePV}></TablePV>
        </div>
      </div>
    );
  }
...
export default connect(mapStateToProps, mapDispatchtoProps)(MethodInjector);
