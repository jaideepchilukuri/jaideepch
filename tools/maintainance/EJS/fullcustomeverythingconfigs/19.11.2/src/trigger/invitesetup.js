...
import { LOGGING, Singletons, _W } from "./top";
...
class InviteSetup {
  constructor(trig, browser, stg, cpps, displayoverride, jrny, triggerMethod, readycallback) {
    ...
    _W.fsReady(() => {
      ...
      // If A/B testing is used for SHORTSURVEY this will alter the inviteType accordingly
      this.abTestShortSurvey(display);

      if (display.inviteType === "SHORTSURVEY") {
        // use the presenter module instead
        _W._fsRequire([makeURI("$fs.shortsurvey.js")], Presenter => {
          const urlgen = new Survey(config, cpps, trig.surveydef);

          this.invite = trig.invite = new Presenter(
            config,
            trig.surveydef,
            display,
            cpps,
      ...
      _W._fsRequire([makeURI("$fs.invite.js")], Invite => {
        ...
        // Set up a new invite
        this.invite = trig.invite = new Invite(
          config,
          trig.surveydef,
          browser,
          display,
    ...
    this.resourcesready.subscribe(
      () => {
        ...
        setTimeout(() => {
          ...
          // The ipt (Invite Presented Time) is used later to calculate how much time passed before
          // the survey loaded in the tracker window.
          stg.set("ipt", currentTime());
  ...
  /**
   * Perform a pinned a/b test for SHORTSURVEY. This will modify the inviteType
   * back to TRACKER if it fails.
   */
  abTestShortSurvey(display) {
    if (
      display.inviteType === "SHORTSURVEY" &&
      display.shortSurvey &&
      display.shortSurvey.abTestPercent &&
      display.shortSurvey.abTestPercent < 100 &&
      display.shortSurvey.abTestPercent > 0
    ) {
      const measureId = new Survey(this.config, this.cpps, this.trig.surveydef).getMeasureId();
      const abKey = `${measureId}-${display.displayname}-AB`;
      let result = this.stg.get(abKey);
      if (result == null) {
        const dice = Math.random() * 100;
        if (dice <= display.shortSurvey.abTestPercent) {
          result = 1;
        } else {
          result = 0;
        }
        ...
        /* pragma:DEBUG_START */
        console.log(`shortsurvey: A/B Test rolled dice: ${dice}`);
        /* pragma:DEBUG_END */

        // pin the result for next time
        this.stg.set(abKey, result);
      }

      // if it fails, do the tracker thing
      if (!result) {
        /* pragma:DEBUG_START */
        console.log(`shortsurvey: A/B Test LOST!`);
        /* pragma:DEBUG_END */

        display.inviteType = "TRACKER";
      } else {
        /* pragma:DEBUG_START */
        console.log(`shortsurvey: A/B Test WON!`);
        /* pragma:DEBUG_END */
      }
    }
  }
...
const startMobileHB = (trigger, userinfo, invitetype) => {
  ...
  const invitePresentedTime = trigger.stg.get("ipt");
  const invitePresentedInterval = (currentTime() - invitePresentedTime) / 1000;
  trigger.cpps.set("invite_presented_interval", invitePresentedInterval);
