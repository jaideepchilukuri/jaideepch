  ...
  canDisplayInvitation() {
    ...
    if (display.inviteType === "SHORTSURVEY" && display.shortSurvey) {
      if (display.shortSurvey.idleViewsBeforeStop > 0) {
        ...
        const presentedCount = this.stg.get(`${surveyMID}pc`) || 0;
        const lastInteraction = this.stg.get(`${surveyMID}li`);

        // When the shortsurvey has never been interacted after it was presented
        if (!lastInteraction && display.shortSurvey.idleViewsBeforeStop <= presentedCount) {
          /* pragma:DEBUG_START */
          console.warn(
