...
class _generalStorage extends _fsStorage {
  ...
  _commit() {
    ...
    if (this.pers == storageTypes.CK) {
      ...
      /* pragma:DEBUG_START */
      // console.log("utils: commit", nval, " to cookie (key: " + this._storageKey + ")", "compression: ", dtastr.length, "B - now is ", compress(dtastr).length, "B");
      /* pragma:DEBUG_END */
      // Simple DOM storage
      this.ckie.set(this._storageKey, compress(dtastr));

      // Fire the event
      this.onCommit.fire(this._data);
    } else if (this.pers == storageTypes.DS) {
      /* pragma:DEBUG_START */
      // console.log("utils: committing fsstorage to localstorage (key: " + this._storageKey + ")", "compression: ", dtastr.length, "B - now is ", compress(dtastr).length, "B");
      /* pragma:DEBUG_END */
      // Simple DOM storage
      localStorage.setItem(this._storageKey, compress(dtastr));
