...
class Presenter {
  constructor(config, surveydef, display, cpps, stg, urlgen) {
    ...
    // The surveyMID must be set before calling getPageView
  ...
  present() {
    ...
    let presentedCount = this.stg.get(`${this.surveyMID}pc`) || 0;
    presentedCount++;
    this.stg.set(`${this.surveyMID}pc`, presentedCount);
  ...
  getPageView() {
    return this.stg.get(`${this.surveyMID}pv`);
  }
