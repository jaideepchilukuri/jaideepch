...
module.exports = ( function () {
     ...
     * @param desc
     */
    ctx.getAnswersCredentials = function (cb, shouldDesc, desc) {
      // Use env.json uberjs credentials if they are available
      try {
        // Check whether to use windows/unix home
        var home = process.env[(process.platform == 'win32') ? 'USERPROFILE' : 'HOME'],
          ev = JSON.parse(fs.readFileSync(home + '/env.json').toString());
        username = ev.UJS_USERNAME;
        password = ev.UJS_PASSWORD;
      } catch (e) {
      }
      if (username && password) {
        process.nextTick(function () {
          cb(username, password, desc || "");
        });
      } else {
        var schema;
        if (shouldDesc) {
        ...
        console.log("Please enter your Answers credentials and a description for the change log. ".cyan);
        prompt.start();
        prompt.get(schema, function (err, result) {
          if (!err) {
            console.log("Thanks, please wait...");
            username = result.userame;
            password = result.password;
            if (result.username.indexOf('@answers.com') == -1) {
              result.username = result.username.trim() + '@answers.com';
            }
            if (!result.description) {
              cb(result.username, result.password);
            } else {
