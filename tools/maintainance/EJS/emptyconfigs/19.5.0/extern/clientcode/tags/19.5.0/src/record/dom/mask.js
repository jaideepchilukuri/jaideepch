...
(function () {
  ...
  Masker.prototype._maskStaticBySelector = function (css_selector, winRef, whiteList) {
    ...
    var docRef = winRef.document,
      ...
      parent,
      wlClass = "_fsrWL";
    this.blockingCommentStart = whiteList ? SESSIONREPLAY_UNBLOCK_START_STRING : SESSIONREPLAY_MASK_START_STRING;
    this.blockingCommentEnd = whiteList ? SESSIONREPLAY_UNBLOCK_END_STRING : SESSIONREPLAY_MASK_END_STRING;
    ...
    for (i = 0; i < nodes_to_mask.length; i++) {
      ...
      if (this_node.nodeName == 'BODY') {
        if (!utils.hasClass(this_node, wlClass)) {
          ...
          this_node.insertBefore(docRef.createComment(this.blockingCommentStart), this_node.firstChild);
          this_node.appendChild(docRef.createComment(this.blockingCommentEnd));
        }
      } else {
  ...
  /**
   * Mask the title tag
   * @param urls {Array} of urls to match
   * @param winRef {Window} Reference the calling window
   * @memberOf Mask
   */
  Masker.prototype._maskTitleTag = function (urls, winRef) {
    if (!urls) {
      return;
    }
    var h = winRef.location.href,
     docRef = winRef.document,
     headTag = docRef.head,
     titleTag = headTag.getElementsByTagName('title')[0];
    for (var i = 0, len = urls.length; i < len; i++) {
      if (utils.testAgainstSearch(urls[i], h)) {
        headTag.insertBefore(docRef.createComment(SESSIONREPLAY_MASK_START_STRING), titleTag);
        headTag.insertBefore(docRef.createComment(SESSIONREPLAY_MASK_END_STRING), titleTag.nextSibling);
        break;
      }
    }
  };
  ...
  Masker.prototype.isNodeMasked = function (pNode) {
    ...
    if (pNode.childNodes.length < 1 || !maskable) {
      return true;
    } else if (pNode.childNodes[0].data && pNode.childNodes[0].data.indexOf(this.blockingCommentStart) > -1) {
      return true;
    }
  ...
  Masker.prototype.maskDocument = function (piiObj, winRef) {
    ...
    this._maskTitleTag(piiObj.blockTitleEl, winRef);
