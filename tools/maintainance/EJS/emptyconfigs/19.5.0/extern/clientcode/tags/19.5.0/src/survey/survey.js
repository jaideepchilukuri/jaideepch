...
(function () {
  ...
  var Survey = function (cfg, cpps, browser) {
    ...
    this.SurveyData.subscribe(
      fs.proxy(function (res, cb) {
        this.data = this._transpileJSONDef(JSON.parse(res));
        this.data.meta.privacyurl = this.cfg.privacyuri;
        this.data.meta.privacytext = this.cfg.privacytext;
        this.data.meta.foreseeLogoAltText = this.defaultCfg.foreseeLogoAltText;
        this.data.meta.exitBtnAltText = this.defaultCfg.exitBtnAltText;
        var ctx = this,
          meta = this.data.meta,
          hasBanner = !!meta.logo2graphic,
          hasLogo = !!meta.logo1graphic,
          gotBanner = false,
          gotLogo = false,
          signal = function () {
            if (cb) {
  ...
  Survey.prototype.bind = function (nd) {
    ...
    for (var i = 0; i < qs.length; i++) {
      ...
      if (cfg) {
        ...
        this.qs.push(q);
        this.qs[this.qs.length - 1].stateChanged.subscribe(fs.proxy(this._renderSurvey, this), false, true);
      }
    }


    // All the things to update when UIUpdated triggers.
    var tmfn = fs.proxy(function () {
      for (var i = 0; i < this.qs.length; i++) {
    ...
    // Show a topic
    var showTopic = fs.proxy(function (tp) {
    ...
    utils.Bind(nd.$('.acs-submit-feedback__button')[0], "click", fs.proxy(function (e) {
