...
import { Browser, Compress, Journey, getBrainStorage, APPID } from "../utils/utils";
    ...
    if (siteKey !== "preview") {
      ...
      brain.watchForChanges(
        ...
        (key, oldfscfg, fscfg) => {
          ...
          cpps = JSON.parse(Compress.decompress(dec(fscfg.cpps)));

          // Parse the configuration
          const global = JSON.parse(dec(Compress.decompress(dec(fscfg.gcfg))));
          popWindowWithConfig(global, mId, cpps, fscfg.cid);
        },
        true,
        true
      );
      ...
      const global = JSON.parse(dec(Compress.decompress(dec(gcfgParam))));
      popWindowWithConfig(global, mId, null, null);
    }
