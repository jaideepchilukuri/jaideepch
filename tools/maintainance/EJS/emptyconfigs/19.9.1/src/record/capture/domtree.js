...
const ___domtree = () => {
   ...
   *   a: optional attributes as an object
  ...
  class DomTree {
    ...
    scan(el) {
      ...
      for (i = 0, len = el.childNodes.length; i < len; i++) {
        // don't add children we already know about because likely
        // they are currently somewhere else in the dom and we have yet
        // to move them
        if (!this.get(el.childNodes[i])) {
      ...
      if (el.nodeName[0] !== "#") {
        ...
        if (el.hasAttributes && el.hasAttributes()) {
          ...
          for (i = 0, len = el.attributes.length; i < len; i++) {
            ...
            node.a[attrName] = el.attributes[i].value;

            // if the URL of the page was changed since the page loaded
            // then relative urls become meaningless. This should produce
            // the absolute url on most elements
            if (
              ...
              node.a[attrName] = el[attrName];
            }
          }

          // Ensure the value is captured for input elements
          if (["INPUT", "SELECT", "TEXTAREA"].indexOf(el.nodeName.toUpperCase()) > -1) {
            node.a.value = el.value;
          }
        }
     ...
     */
    add(parentEl, el) {
      const parent = this.get(parentEl);
      ...
      if (index < 0) {
        throw new Error("oops, couldn't find child!");
      }
      /* pragma:DEBUG_END */

      const node = this.scan(el);
     ...
     */
    insert(index, node) {
      const parent = this.getById(node.p);
     ...
     */
    move(el, newParentEl, prevSibling, nextSibling) {
      const node = this.get(el);
      const parent = this.getById(node.p);
      const newParent = this.get(newParentEl);
      let idx;
      ...
      if (!prevSibling) {
        ...
        if (nextSibling && this.get(nextSibling) === newParent.c[0]) {
          ...
          idx = newParent.c.indexOf(this.get(nextSibling));
     ...
     */
    moveById(id, newParentId, childIndex) {
      const node = this.getById(id);
      const oldParent = this.getById(node.p);
      const newParent = this.getById(newParentId);
    ...
    remove(el) {
      ...
      for (i = node.c.length - 1; i >= 0; i--) {
        /* pragma:DEBUG_START */
        if (!node.c[i]) {
          throw new Error(`Child ${i} of ${node.id} is undefined!`);
        }
        /* pragma:DEBUG_END */
...
const DomTree = ___domtree();

export { ___domtree, DomTree };
