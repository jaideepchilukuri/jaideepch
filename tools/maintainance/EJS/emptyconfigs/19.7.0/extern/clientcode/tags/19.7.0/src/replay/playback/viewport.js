...
fs.provide("rp.Replay.Playback.Viewport");
...
(function () {
  ...
  Viewport.prototype.update = function (params) {
    ...
    // check size
    if (
      oldParams.vw !== this.params.vw || oldParams.vh !== this.params.vh ||
      oldParams.lw !== this.params.lw || oldParams.lh !== this.params.lh ||
      this.forceUpdate
    ) {
      change = true;
      this.resize();
    ...
    // check position
    if (
      oldParams.vx !== this.params.vx || oldParams.vy !== this.params.vy
    ) {
      change = true;
    }
