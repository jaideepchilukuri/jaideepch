...
(function (trigger) {
  ...
  Trigger.prototype.init = function () {
    ...
    if (fs.isDefined(olddef) && this.isTrackerAlive()) {
      ...
      /* pragma:DEBUG_START */
      // I don't know in which case that would happen, if ever. So it would be interesting to know
      if (this.stg._data.keys.tracker_hb.v !== this.stg._data.keys.def.v) {
        console.error("Tracker's definition !== olddef", this.stg._data.keys.tracker_hb.v, this.stg._data.keys.def.v);
      }
      console.warn("trigger: no valid survey def, but a Tracker window is still alive, with def:", def);
      /* pragma:DEBUG_END */
  ...
  Trigger.prototype.isTrackerAlive = function () {
    var trackerHb = this.stg.get('tracker_hb');
    return fs.isDefined(trackerHb);
  };
