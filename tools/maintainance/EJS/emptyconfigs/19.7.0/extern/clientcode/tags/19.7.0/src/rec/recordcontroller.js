...
(function () {
  ...
  function RecordController() {
    // TODO clean this up: args, correct journey args, checking for feedback and trigger values
    var storageArg = arguments[2];

    this.jrny = Singletons.jrny = new utils.Journey(
      //customerId, appId, userId, browser
      fs.config.customerId || utils.getRootDomain() || 'record_customerId',
      utils.APPID.REPLAY,
      storageArg.uid || storageArg.get("rid") || 'record_userId',
      arguments[0]
    );
    ...
    if (controllerInstance !== null) {
      console.error("sr: " + (window === window.top ? '' : '[frm] ') + "Cannot instantiate more than one instance of RecordController. Use getInstance().");
    }
      ...
      browser.ready.subscribe(function () {
        ...
        if (this.crit.supported() || this.crit.didSkipOnPurpose) {
          ...
          if (!isCrossDomainFrame) {
            ...
            this.recorder = new Recorder(stg, browser, winobj, 0, null, recordconfig, null, isCrossDomainFrame, null, null, null, null, !!this.crit.didSkipOnPurpose);
            this.recorder.ready.subscribe(function () {
              cpps.set('replay_id', this.recorder.sesh.sessionInfo[SESSION_SYMBOLS.GLOBALSESSIONID]);
              cpps.set('sessionid', this.recorder.sesh.sessionInfo[SESSION_SYMBOLS.SESSIONID]);
            ...
            utils.Bind(window, 'beforeunload', function () {
              /* pragma:DEBUG_START */
              console.log("sr: " + (window === window.top ? '' : '[frm] ') + "unloading", this.recorder);
              /* pragma:DEBUG_END */
              if (this.recorder) {
              ...
              if (ed.data && fs.isFunction(ed.data.indexOf) && ed.data.length > 3 && ed.data.indexOf('{') > -1) {
                ...
                if (!(dt.src && dt.src == 'fsframe')) {
                  ...
                  if (dt.cxr && dt.id) {
                    ...
                    // Set up a new recorder
                    this.recorder = new Recorder(stg, browser, winobj, dt.xp, null, recordconfig, null, isCrossDomainFrame, dt.id, ed.source, dt.sid, dt.sp);
                    this.recorder.ready.subscribe(function () {
                      cpps.set('replay_id', this.recorder.sesh.sessionInfo[SESSION_SYMBOLS.GLOBALSESSIONID]);
                      cpps.set('sessionid', this.recorder.sesh.sessionInfo[SESSION_SYMBOLS.SESSIONID]);
  ...
  RecordController.prototype.sendCustomEvent = function (type, defmsg, msg, meta) {
    return this.recorder &&
      this.recorder.sendCustomEvent(type, defmsg, msg, meta);
  };
