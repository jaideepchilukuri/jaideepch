...
(function () {
  ...
  var PRELOAD_TIMEOUT = 30000;
  var STYLE_REGEX = /(fonts\.googleapis\.com\/css|\.css)\b/i;
  var IMAGE_REGEX = /\.(jpg|jpeg|jpe|gif|svg|png|tiff|webp|ico)\b/i;
  var VIDEO_REGEX = /\.(webm|mp4|m4v|m4p|mov|mpg|mpeg)\b/i;
  var FONT_REGEX = /\.(woff|woff2|ttf|oft)\b/i;
  ...
  Worker.prototype.load = function (initialDOM, viewportParams, assets) {
    ...
    return this.preloadAssets(assets).then(function () {
      return this.frames[0].load(initialDOM, scrollPos);
  ...
  /**
   * Preload the assets that will be used throughout the replay
   */
  Worker.prototype.preloadAssets = function (assets) {
    ...
    return new Promise(function (resolve) {
      ...
      assets.forEach(function (asset) {
        var dbgmsg = { url: asset.url, proxy: asset.proxyUrl };
        // These are all kind of bugs in session processor, these urls shouldn't be
        // translated to proxy urls
        if (
          asset.url === "favicon.ico" ||
          asset.url === (asset.base + '/') ||
          asset.url === "#" ||
          asset.url === ""
        ) {
          remainingCount--;
        ...
        var kind;
        if (asset.url) {
          if (asset.url.match(STYLE_REGEX)) {
            kind = "style";
          } else if (asset.url.match(IMAGE_REGEX)) {
            kind = "image";
          } else if (asset.url.match(FONT_REGEX)) {
            kind = "font";
          }
        }
        if (!kind) {
          // can't figure out what this is
          console.warn("Unable to determine type of this asset so not preloading it:", dbgmsg);
          remainingCount--;
          return;
        }
        ...
        var timeout = setTimeout(function () {
          console.error("Timed out preloading", dbgmsg);
          done();
        }, PRELOAD_TIMEOUT);
        var error = function () {
          console.error("Error preloading", dbgmsg);
          done();
        };
        var done = function () {
        ...
        el.setAttribute("href", asset.proxyUrl);
