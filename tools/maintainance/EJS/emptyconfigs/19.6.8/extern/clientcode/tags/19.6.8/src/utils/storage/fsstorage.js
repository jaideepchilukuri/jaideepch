...
(function (utils) {
  ...
  var _fsStorage = function (browser, uidoverride) {
    ...
    fs.ext(this, {
      ...
      // true if one of the data keys has expired
      isStale: false,
  ...
  _fsStorage.prototype._fireChangeEvents = function (newData) {
    ...
    if (changedKeys.length > 0) {
      // Further lower console spam from the heartbeat
      // (Chrome's console recognises same lines and squashes them)
      console.log(
        "utils: firing change events on", "[" + changedKeys.toString() + "]",
        (changedKeys.length === 1 && changedKeys[0] === "page_hb") ? "♡" : newData,
        this.uid
      );
    }
    /* pragma:DEBUG_END */
  ...
  _fsStorage.prototype.save = function (emergency) {
    if (emergency) {
      ...
      if (!this.lock && this.isStale) {
        this.lock = setTimeout(this._commit.bind(this), Math.max(0, this.cThreshold - (nw - this.lastSave)));
      }
    }
  };
  ...
  _fsStorage.prototype.erase = function (key, callback, emergency) {
    if (Array.isArray(key)) {
      ...
      if (Singletons.StorageInstances.brainStorage) {
        // Part of brainStorage subclass
        this._delete(key);
      }
  ...
  _fsStorage.prototype.reset = function (success, failure, keepUID) {
    ...
    if (!keepUID) {
      ...
      if (this.browser.supportsSessionStorage) { window.sessionStorage.removeItem(this._microStorageKey, this.uid); }
    }
  ...
  utils.getGeneralStorage = function (browser, uidoverride) {
    ...
    if (!(pers == st.MC || pers == st.CL || pers == st.DS || pers == st.CK)) {
      console.error('utils: invalid persistence setting. Must be \'' + st.CK + '\', \'' + st.MC + '\', \'' + st.CL + '\', or \'' + st.DS + '\'.');
    }
    /* pragma:DEBUG_END */
