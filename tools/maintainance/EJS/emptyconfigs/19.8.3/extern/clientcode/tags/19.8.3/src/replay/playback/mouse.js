...
Mouse.prototype.updateMousePos = function() {
  ...
  this.el.style.transform =
    "translate(" + (pt.x - mouseImageData.hit.x) + "px," + (pt.y - mouseImageData.hit.y) + "px)";
};
...
Mouse.prototype.click = function(pos) {
  ...
  el.style.transform =
    "translate(" + (pt.x - clickImageData.hit.x) + "px," + (pt.y - clickImageData.hit.y) + "px)";
  ...
  var fadeAnim = new Animator({
    ...
    frameCallback: function(val) {
      ...
      el.style.transform =
        "translate(" +
        (pt.x - clickImageData.hit.x) +
        "px," +
        (pt.y - clickImageData.hit.y) +
        "px)";
    }.bind(this),
  });
...
Mouse.prototype.touchUpdate = function(pts) {
  ...
  for (var i = 0; i < this.touchEls.length; i++) {
    ...
    this.touchEls[i].style.transform =
      "translate(" + (pt.x - touchImageData.hit.x) + "px," + (pt.y - touchImageData.hit.y) + "px)";
  }
};
...
Mouse.prototype.handleHovering = function() {
  ...
  if (this.visible) {
    ...
    var vx = this.pos.x - scrollPos.x,
      vy = this.pos.y - scrollPos.y;
    ...
    if (hoverEl && hoverEl.tagName === "IFRAME") {
      try {
        ...
        var cx = vx - Math.round(rect.x),
          cy = vy - Math.round(rect.y);
