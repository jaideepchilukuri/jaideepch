...
utils.testSameDomain = function(url1, url2) {
  ...
  return (
    fs.toLowerCase(hostname1) == fs.toLowerCase(hostname2) &&
    fs.toLowerCase(protocol1) == fs.toLowerCase(protocol2)
  );
};
...
utils.getRootDomain = function(url) {
  url = fs
    .toLowerCase(url || document.domain)
    .replace("https://", "")
    .replace("http://", "");
  var iposList = ["/", "?", ":"],
  ...
  var ubits = url.split(".");
  var ubl = ubits.length;
  var lastTwo = ubl > 1 ? ubits[ubl - 2] + "." + ubits[ubl - 1] : url;
