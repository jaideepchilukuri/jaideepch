...
Trigger.prototype.popSurvey = function(serverStatus) {
  ...
  if (!this.didPopTrackerAlready) {
    ...
    if (this.stg && this.stg.get("page_hb")) {
      ...
      utils.getBrainStorage(this.browser, this.stg.uid).set(
        "trackercmd",
        {
          method: "survey",
        },
...
Trigger.prototype.init = function() {
  ...
  if (
    !fs.isDefined(olddef) ||
    defs[parseInt(olddef)].selectMode == "default" ||
    defs[parseInt(olddef)].selectMode == "pin"
  ) {
    // Loop over the defs and choose one, hopefully
    for (
      i = 0;
      i <
      (fs.isDefined(olddef) && defs[parseInt(olddef)].selectMode != "default"
        ? parseInt(olddef) + 1
        : defs.length);
      i++
    ) {
      ...
      if (
        (fs.isDefined(olddef) && olddef == i && defs[parseInt(olddef)].selectMode != "default") ||
      ...
      case "url":
        ...
        if (fs.isDefined(ll)) {
          for (var j = 0, len = ll.length; j < len; j++) {
            if (
              fs.isDefined(ll[j].locale) &&
              fs.isDefined(ll[j].match) &&
