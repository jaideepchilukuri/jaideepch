...
const PromisePolyfill = (function() {
  ...
  Promise._unhandledRejectionFn = function _unhandledRejectionFn(err) {
    /* pragma:DEBUG_START */
    console.error("Possible Unhandled Promise Rejection:", err); // eslint-disable-line no-console
    /* pragma:DEBUG_END */
  };
...
const MaybeNativePromise = _W.Promise || PromisePolyfill;
const isIE = /MSIE |Trident/.test(window.navigator.userAgent);

// on IE always use our bundled polyfill not a 3rd party. Otherwise try to use
// the browser's Promise implementation (which will be faster).
const Promise = isIE ? PromisePolyfill : MaybeNativePromise;
