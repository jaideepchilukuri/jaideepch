...
import { getNested } from "../../fs/util/getnested";
...
function applyQAOverrides() {
  ...
  eachProp(ext({ feedback: null }, productConfig), (originalConfig, name) => {
    if (overrides[name]) {
...
function enterQAMode(bootSDK) {
  ...
  _fsRequire([_fsNormalizeUrl("$fs.utils.js")], function(utils) {
    defineQAAPI(
      bootSDK,
      utils,
      ext({ trigger: null, feedback: null, record: null }, productConfig)
    );
  });
}
...
export { shouldEnterQAMode, applyQAOverrides, enterQAMode };
