...
/* eslint-disable es5/no-es6-methods, es5/no-es6-static-methods, es5/no-for-of */
...
if (
  (taskName === "test_debug" || taskName === "test_prod" || process.argv.includes("--dev")) &&
...
function requireConfig(filename) {
  ...
  try {
    ...
    console.error(chalk.red(`Failed to load config "${fullpath}"`));
    throw e;
  }
}
...
const getGlobalConfig = envNumber => {
  ...
  globalConfig.cookieExpiration = clientProperties.client.cookieExpiration || 730; // Default to 2 years / 730 days
...
function getAllConfigs(envNumber) {
  ...
  // in the future, to add a product config, just add it here
  const products = ["feedback", "fbmods", "record", "trigger"];
  const configs = {
    products,
    global: globalConfig,
  };
  ...
  for (const product of products) {
    configs[product] = requireConfig(
      `${pjson.build.config}/productconfig/${product}/product_config.js`
    );
  }
  ...
  const svDefFiles = fs
    ...
    .filter(function(file) {
      // Make sure we're only reading files, non-dot-files (.DS_Store)
      return (
        !/^\./.test(file) &&
        fs
  ...
  // special trigger config handling
  configs.trigger = {
    config: configs.trigger,
