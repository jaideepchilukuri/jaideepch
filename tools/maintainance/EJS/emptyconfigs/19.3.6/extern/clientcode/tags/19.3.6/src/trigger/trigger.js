...
(function (trigger) {
  ...
  var Trigger = function (stg, cfg, browser, crit, cpps, jrny) {
    ...
    if (!stg.get('pv')) {
      ...
      cppsToSet = {
        ...
        'code': fs.config.codeVer,
        'fp': browser.fp,
        'os': browser.os.name,
        'referrer': document.referrer.toString(),
        'site': cfg.config.site_id,
      ...
      if (invite) {
        ...
        var ist = new InviteSetup(this, ctx.browser, ctx.stg, ctx.cpps, false, ctx.jrny, function () {
          ...
          if (ctx.surveydef.cxRecord && Singletons.rec && Singletons.rec.recorder) {
            ...
            // Set the CPPS
            ctx.cpps.set('replay_id', Singletons.rec.recorder.logger.gsessionid);
            ctx.cpps.set('sessionid', Singletons.rec.recorder.logger.sessionid);
          } else {
            /* pragma:DEBUG_START */
  ...
  Trigger.prototype.logDefState = function () {
    ...
    if (this.surveydef) {
      this.defPageViewCount = (this.stg.get(this.surveydef.name + 'pv') || 0) + 1;
      this.stg.set(this.surveydef.name + 'pv', this.defPageViewCount, config.config.pageViewsResetTimeout || 24 * 60 * 60 * 1000);
    }
  };
