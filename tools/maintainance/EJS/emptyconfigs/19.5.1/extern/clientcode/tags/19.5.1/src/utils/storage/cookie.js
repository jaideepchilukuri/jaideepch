...
(function (utils) {
  ...
  utils.Cookie.prototype.set = function (key, value, opts) {
    ...
    // Apply "secure" flag (to lock cookie to HTTPS). The flag has to exist and be set to something different than "false". This should only apply in cases where hasSSL.
    if (fs.config.cookieSecure && fs.config.cookieSecure !== "false" && fs.hasSSL !== "false") {
      value += ";secure";
    }
  ...
  utils.Cookie.prototype.kill = function (cookieName) {
    ...
    this.set(cookieName, '', {'expires': expireDate});
  };
