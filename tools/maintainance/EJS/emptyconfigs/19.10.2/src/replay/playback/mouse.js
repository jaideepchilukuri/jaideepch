 ...
 */
class Mouse {
  constructor(viewport, animWin, iframe) {
  ...
  setVisible(visible) {
    ...
    const word = this.visible ? "visible" : "hidden";
  ...
  transformCoord(coord) {
    ...
    const viewBounds = this.iframe.getBoundingClientRect();
    let pageBounds;
    ...
    const iw = viewBounds.width;
    const ih = viewBounds.height;
  ...
  updateMousePos() {
    ...
    const pt = this.transformCoord(this.pos);

    // Perform the mouse movements on the GPU by using transform
    this.el.style.transform = `translate(${pt.x - mouseImageData.hit.x}px,${pt.y -
      mouseImageData.hit.y}px)`;
  }
   ...
   */
  click(pos) {
    const pt = this.transformCoord(pos);
    ...
    el.style.transform = `translate(${pt.x - clickImageData.hit.x}px,${pt.y -
      clickImageData.hit.y}px)`;
  ...
  touchUpdate(pts) {
    ...
    for (let i = 0; i < this.touchEls.length; i++) {
      const pt = this.transformCoord(this.touchPts[i]);
      this.touchEls[i].style.transform = `translate(${pt.x - touchImageData.hit.x}px,${pt.y -
        touchImageData.hit.y}px)`;
    }
  }
  ...
  touchEnd() {
    ...
    for (let i = 0; i < this.touchPts.length; i++) {
  ...
  handleHovering() {
    ...
    if (this.visible) {
      ...
      const scrollPos = getScroll(this.iframe.contentWindow);
      const vx = this.pos.x - scrollPos.x;
      const vy = this.pos.y - scrollPos.y;
      ...
      if (hoverEl && hoverEl.tagName === "IFRAME") {
        try {
          ...
          const cx = vx - Math.round(rect.x);
          const cy = vy - Math.round(rect.y);
   ...
   * @private
   */
  animate(spline, startTime) {
    this.win.requestAnimationFrame(time => {
      startTime = startTime || time;
      const splineTime = time - startTime;
  ...
  playFrame(spline, time) {
    ...
    const t = time / spline.timeTillNext;
    const bz = interpolateBezier(t, spline.start, spline.ctp1, spline.ctp2, spline.end);
