...
Upgrader.prototype.Upgrade = function(cb) {
  console.log(
    "Template upgrader script".yellow +
      " - ".grey +
      " Should be run with sudo or Administrator access!".magenta
  );
...
*/
  ...
  if (!fs.existsSync("./clientconfig/foresee/surveydef/def1.js")) {
    console.log(
      "Error. You are not dealing with a pristine template. Check it out fresh and try again!".red
    );
    cb();
    return;
  }
          ...
          svnsync(
            ...
            function(err) {
              ...
              if (!fsgutil.fileExistsSync(fullSrcPath + "/package.json")) {
                ...
                console.log(
                  "Probable code version: ".yellow +
                    " " +
                    codeVersion.toString().magenta +
                    "...".yellow
                );
...
Upgrader.prototype._upgrade19x = function(fullSrcPath) {
  ...
  if (
    fsgutil.fileExistsSync(fullSrcPath + "/assets/sitelogo.gif") ||
    fsgutil.fileExistsSync(fullSrcPath + "/assets/sitelogo.png")
  ) {
    if (debugMode) {
      ...
      if (fs.existsSync(fullSrcPath + "/assets/sitelogo.gif")) {
        ...
        logoFile = "sitelogo.gif";
        fsgutil.moveFileIfExists(
          fullSrcPath + "/assets/sitelogo.gif",
          "./assets/foresee/sitelogo.gif"
        );
      }
      ...
      if (fs.existsSync(fullSrcPath + "/assets/sitelogo.png")) {
        ...
        logoFile = "sitelogo.png";
        fsgutil.moveFileIfExists(
          fullSrcPath + "/assets/sitelogo.png",
          "./assets/foresee/sitelogo.png"
        );
      }
    }
  }
  ...
  if (fsgutil.fileExistsSync(fullSrcPath + "/clientconfig/client_properties.js")) {
    ...
    var finalstr =
      "// Generated with client code converter\nmodule.exports = " +
      JSON.stringify(nprops, null, 2) +
      ";";
    if (fsgutil.fileExistsSync("./clientconfig/client_properties.js")) {
      if (!debugMode) {
        fs.writeFileSync("./clientconfig/client_properties.js", Buffer.from(finalstr));
        ctx._logStep(
          "Converted client properties (client id: " +
            nprops.client.id +
            ", replayid: " +
            nprops.client.replayid +
            ", siteid: " +
            nprops.client.siteid +
            ", uberid: " +
            nprops.client.uberid +
            ")"
        );
      } else {
        ctx._logStep(
          "Converted client properties (client id: " +
            ...
            ", uberid: " +
            nprops.client.uberid +
            ") but did not write to file system due to debug mode"
        );
      }
    }
    ...
    if (hasReplay) {
      ...
      if (!debugMode) {
        ...
        ctx._logStep(
          "Converted cxReplay configuration but did not write to disk due to debug mode"
        );
      }
    }
    ...
    if (fsgutil.fileExistsSync(fullSrcPath + "/clientconfig/trigger_config.js")) {
      ...
      var tconf = fs
        .readFileSync(fullSrcPath + "/clientconfig/trigger_config.js")
        .toString("utf-8");
      ...
      if (tconf && sdefs) {
        ...
        finalCfg =
          atob(
            "LyoqDQogKiBGb3JlU2VlIFRyaWdnZXIgQ29uZmlndXJhdGlvbiAqKioqKioqKioqKioqKioqKioqKioqKioqKioqKioqKioqKioqKioqKioqKioqKioqKioqKg0KICovDQp2YXIgdHJpZ2dlcmNvbmZpZyA9IA=="
          ) +
          finalCfg +
          "\n" +
          atob(
            ""
          );
        ...
        for (var g = 0; g < sdefs.length; g++) {
          ...
          if (!debugMode) {
            ...
            fs.writeFileSync(
              "./clientconfig/foresee/surveydef/" + fname,
              "(" + fsgutil.recodeRegexpFromString(fsgutil.JSONStringifyWithRegexp(defInst)) + ")"
            );
          } else {
...
Upgrader.prototype._upgrade18x = function(fullSrcPath) {
  ...
  if (fsgutil.fileExistsSync(fullSrcPath + "/client.properties.js")) {
    ...
    fsjs = fsjs
      .replace(/:[ ]*(\${[^\}]*})[ ]*,/g, ": '$1',")
      .replace(/\$\{client\.acceptableorigins\}/g, "'${client.acceptableorigins}'");
    ...
    var triggerDelay = parseInt(
        fsgutil.regexMatchGroup(/triggerDelay[\W]*:[\W]*([0-9]*)[\W]*,/gi, 0, fsjs)
      ),
      layout = fsgutil.regexMatchGroup(
        /layout[\W]*:[\W]*['"]([^'"]*)['"][\W]*,/gi,
        "CENTERFIXED",
        fsjs
      ),
    ...
    if (!isNaN(triggerDelay)) {
      prodcfg = prodcfg.replace(
        /triggerDelay[\W]*:[\W]*[0-9]*[\W]*,/gi,
    ...
    if (hasReplay) {
      ...
      var replaycfg = fs
          .readFileSync("./clientconfig/cxreplay/product_config.js")
          .toString("utf-8"),
        oldreplaycfg = fs.readFileSync(fullSrcPath + "/sessionrecord/config.js").toString("utf-8");

      oldreplaycfg = stripComments(oldreplaycfg);

      var staticBlockEls = fsgutil.regexMatchGroup(
          /staticBlockEls[\W]*:[\W]*{([^}]*)}[\W]*,/gi,
          "",
          oldreplaycfg
        ),
        dynamicBlockEls = fsgutil.regexMatchGroup(
          /dynamicBlockEls[\W]*:[\W]*{([^}]*)}[\W]*,/gi,
          "",
          oldreplaycfg
        ),
        staticVisibleEls = fsgutil.regexMatchGroup(
          /staticVisibleEls[\W]*:[\W]*{([^}]*)}[\W]*,/gi,
          "",
          oldreplaycfg
        ),
        dynamicVisibleEls = fsgutil.regexMatchGroup(
          /dynamicVisibleEls[\W]*:[\W]*{([^}]*)}[\W]*,/gi,
          "",
          oldreplaycfg
        ),
        assetBlockEls = fsgutil.regexMatchGroup(
          /assetBlockEls[\W]*:[\W]*{([^}]*)}[\W]*,/gi,
          "",
          oldreplaycfg
        ),
        removeVisibilityEls = fsgutil.regexMatchGroup(
          /removeVisibilityEls[\W]*:[\W]*{([^}]*)}[\W]*,/gi,
          "",
          oldreplaycfg
        ),
        obscureEls = fsgutil.regexMatchGroup(
          /obscureEls[\W]*:[\W]*{([^}]*)}[\W]*,/gi,
          "",
          oldreplaycfg
        );

      var platWindows = fsgutil.extractRegexBool(
          /platforms[\W]*:[\W]*{[^}]*windows['"]*:[\W]*([^,]*),/gi,
          false,
          oldreplaycfg
        ),
        platMac = fsgutil.extractRegexBool(
          /platforms[\W]*:[\W]*{[^}]*mac['"]*:[\W]*([^,]*),/gi,
          false,
          oldreplaycfg
        ),
        platLinux = fsgutil.extractRegexBool(
          /platforms[\W]*:[\W]*{[^}]*linux['"]*:[\W]*([^,]*),/gi,
          false,
          oldreplaycfg
        ),
        platIpod = fsgutil.extractRegexBool(
          /platforms[\W]*:[\W]*{[^}]*ipod['"]*:[\W]*([^,]*),/gi,
          false,
          oldreplaycfg
        ),
        platIpad = fsgutil.extractRegexBool(
          /platforms[\W]*:[\W]*{[^}]*pad['"]*:[\W]*([^,]*),/gi,
          false,
          oldreplaycfg
        ),
        platIphone = fsgutil.extractRegexBool(
          /platforms[\W]*:[\W]*{[^}]*iphone['"]*:[\W]*([^,]*),/gi,
          false,
          oldreplaycfg
        ),
        platAndroid = fsgutil.extractRegexBool(
          /platforms[\W]*:[\W]*{[^}]*android['"]*:[\W]*([^,]*),/gi,
          false,
          oldreplaycfg
        ),
        platBlackberry = fsgutil.extractRegexBool(
          /platforms[\W]*:[\W]*{[^}]*blackberry['"]*:[\W]*([^,]*),/gi,
          false,
          oldreplaycfg
        ),
        platWinphone = fsgutil.extractRegexBool(
          /platforms[\W]*:[\W]*{[^}]*winphone['"]*:[\W]*([^,]*),/gi,
          false,
          oldreplaycfg
        ),
        platKindle = fsgutil.extractRegexBool(
          /platforms[\W]*:[\W]*{[^}]*kindle['"]*:[\W]*([^,]*),/gi,
          false,
          oldreplaycfg
        ),
        platNook = fsgutil.extractRegexBool(
          /platforms[\W]*:[\W]*{[^}]*nook['"]*:[\W]*([^,]*),/gi,
          false,
          oldreplaycfg
        ),
        platWince = fsgutil.extractRegexBool(
          /platforms[\W]*:[\W]*{[^}]*wince['"]*:[\W]*([^,]*),/gi,
          false,
          oldreplaycfg
        ),
        platMobile = fsgutil.extractRegexBool(
          /platforms[\W]*:[\W]*{[^}]*mobile['"]*:[\W]*([^,]*),/gi,
          false,
          oldreplaycfg
        ),
        platOther = fsgutil.extractRegexBool(
          /platforms[\W]*:[\W]*{[^}]*other['"]*:[\W]*([^,]*),/gi,
          false,
          oldreplaycfg
        );
      ...
      replaycfg = replaycfg.replace(
        /\/\*device_type_support[^\/]*\//gi,
        "device_type_support: " + JSON.stringify(deviceSupportObj, null, 2) + ","
      );
      ctx._logStep(
        "Read platform support settings and applied this to record config: " +
          fsgutil.compressJSString(JSON.stringify(deviceSupportObj))
      );
      ...
      ctx._logStep(
        "Converted dynamicBlockEls: {" + fsgutil.compressJSString(dynamicBlockEls) + "}"
      );

      replaycfg = replaycfg.replace(
        /staticVisibleEls[\W]*:[^,]*/i,
        "\nstaticVisibleEls: {" + staticVisibleEls + "}"
      );
      ctx._logStep(
        "Converted staticVisibleEls: {" + fsgutil.compressJSString(staticVisibleEls) + "}"
      );

      replaycfg = replaycfg.replace(
        /dynamicVisibleEls[\W]*:[^,]*/i,
        "\ndynamicVisibleEls: {" + dynamicVisibleEls + "}"
      );
      ctx._logStep(
        "Converted dynamicVisibleEls: {" + fsgutil.compressJSString(dynamicVisibleEls) + "}"
      );
      ...
      replaycfg = replaycfg.replace(
        /removeVisibilityEls[\W]*:[^,]*/i,
        "\nremoveVisibilityEls: {" + removeVisibilityEls + "}"
      );
      ctx._logStep(
        "Converted removeVisibilityEls: {" + fsgutil.compressJSString(removeVisibilityEls) + "}"
      );
    ...
    var oldsurveydef = fs
      .readFileSync(fullSrcPath + "/trigger/foresee_surveydef.js")
      .toString("utf-8");
    ...
    ctx._logWarning(
      "Note: DID NOT convert events from /trigger/config.js due to basic incompatibility"
    );
    ...
    if (FSR) {
      ...
      if (props.repeatdays) {
        prodcfg = prodcfg.replace(
          /repeatDays[^}]*}/gi,
          "repeatDays: {decline:" + props.repeatdays + ",accept:" + props.repeatdays + "}"
        );
      ...
      if (props.invite) {
        ...
        if (iprops.exclude) {
          prodcfg = prodcfg.replace(
            /inviteExclude[^}]*}/gi,
            "inviteExclude: " + fsgutil.JSONStringifyWithRegexp(iprops.exclude)
          );
          ctx._logStep(
            "Converted invite exclude: " +
              fsgutil.compressJSString(fsgutil.JSONStringifyWithRegexp(iprops.exclude))
          );
        }
      }
      ...
      if (props.cpps) {
        prodcfg = prodcfg.replace(
          /cpps[\W]*:[\W]*{[\W]*\/\*[\d\w\W]*}\*\/[^}]*}/gi,
          "cpps: " + fsgutil.JSONStringifyWithRegexp(props.cpps)
        );
    ...
    if (FSR.surveydefs) {
      ...
      for (var i = 0; i < FSR.surveydefs.length; i++) {
        ...
        if (olddef.platform.toLowerCase() == "tablet") {
          ...
          isMobile = true;
          ctx._logStep(
            "Survey definition #" + (i + 1) + " is a tablet and phone definition. Converting"
          );
        } else {
          defObj.criteria.supportsTablets = false;
          defObj.criteria.supportsSmartPhones = false;
        ...
        if (olddef.criteria) {
          ...
          ctx._logStep(
            " - Converted criteria object: " +
              fsgutil.compressJSString(fsgutil.JSONStringifyWithRegexp(olddef.criteria))
          );
        }
        ...
        if (olddef.include) {
          ...
          ctx._logStep(
            " - Applying include rule: " +
              fsgutil.compressJSString(fsgutil.JSONStringifyWithRegexp(olddef.include))
          );
        }
    ...
    ctx._logWarning(
      "Note: no effort was made to translate any qualifier pages you may have set up"
    );
