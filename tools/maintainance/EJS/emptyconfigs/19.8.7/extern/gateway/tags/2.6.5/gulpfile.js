...
if (taskName && taskName.indexOf("_test_envs") > -1) {
  testEnvs = true;
}
...
gulp.task("code", ["clean"], function(cb) {
  ...
  if (!forceDebug) {
    console.log("***********************************************************".magenta);
    console.log(
      "*******".magenta + " PROD CODE".yellow + "        **********************************".magenta
    );
    console.log("***********************************************************".magenta);
  } else {
    console.log("***********************************************************".magenta);
    console.log(
      "*******".magenta + " DEBUG CODE".gray + "         ********************************".magenta
    );
    console.log("***********************************************************".magenta);
  }

  console.log(
    "Building gateway ".grey +
      pJSON.version.toString().yellow +
      " with a default code version of ".grey +
      defaultCodeVer.toString().yellow +
      ".".grey
  );
  ...
  if (forceDebug) {
    ...
    return gulp
      .src(paths.gatewaysrc)
      .pipe(concat("gateway.min.js"))
      .pipe(pragma({ debug: forceDebug || false }))
      .pipe(replace(/\$\{defaultCodeVer\}/g, defaultCodeVer))
      .pipe(wrap("(function(){\n<%= contents %>\n})();"))
      .pipe(
        uglify({
...
gulp.task("dev", ["code"], function(cb) {
  ...
  var globalConfig =
      "globalConfig = " +
      fs.readFileSync("./" + paths.test + "sample_global_config.json").toString() +
      ";",
  ...
  var startServerSSL = function() {
    handydevserver(443, testfolders, {
      ssl: true,
      ignore: ignorelist,
      latency: 0,
    ...
    fsGulpUtil.signal(
      "SSL Server Started",
      "Build complete. Web server running at https://localhost..."
    );
  };

  // Start a non-SSL server
  var startServer = function() {
    handydevserver(80, testfolders, {
      ssl: false,
      ignore: ignorelist,
      latency: 0,
      ontextfile: textfilehandler,
...
var DoPublish = function(u, p, env, notes, cb) {
  ...
  fcp.postGatewayFiles(uniminfiedGW, minifiedGW, notes, function(success) {
    envMatch = env.match(re) ? env.match(re)[1] : "prod";
    if (!success) {
