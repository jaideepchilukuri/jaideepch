...
(function (utils) {
   ...
   * @constructor
   */
  utils.Journey = function (customerId, appId, stg, browser, threshold) {
    // Save the CID
    ...
    this.data = {
      ...
      'appId': appId || "NULL",
      'userId': stg.uid || stg.get('rid') || "00000000-0000-0000-0000-00000000000",
      'deviceProfile': {
  ...
  utils.Journey.prototype.updateVisitId = function () {
    if (!this.stg.set) {
      // we are in a context that does not have stg available
      return;
    }

    var visitId = this.stg.get('vi');
    if (!visitId) {
      visitId = utils.generateGUID();
    }

    // set every time we use it to reset the expiry time
    // todo: different expiry on mobile?
    this.stg.set('vi', visitId, VISIT_EXPIRY_DESKTOP);
