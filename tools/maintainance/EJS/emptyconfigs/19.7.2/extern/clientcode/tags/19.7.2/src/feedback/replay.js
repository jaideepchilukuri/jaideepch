  ...
  var Replay = {
    ...
    setup: function (browser, cpps, inst, callback) {
      ...
      if ((fs.isDefined(fs.config.products.record) && fs.config.products.record === false) || !fs.productConfig.record) {
        ...
        require([fs.makeURI(rec), 'recordconfig'], function (RecordController, recordconfig) {
          // Keep a local copy
          ...
          // Make a storage unit
          if (!_recordStg) {
            _recordStg = utils.getGeneralStorage(browser);
          }

          var extraSettings = {
            id: fs.config.customerId || utils.getRootDomain() || 'feedback_customerId',
          };

          // Set up an instance of the recorder
          _recordController = RecordController.getInstance(browser, window, _recordStg, extraSettings, cpps);
