...
(function () {
  ...
  var InputCapture = function (recorder, ipt) {
    ...
    utils.Bind(ipt, "record:scroll", function () {
    ...
    // Bind to focus event
    utils.Bind(ipt, "record:focus", function () {
      // Don't do anything if recorder doesn't exist
      if (!this.input) {
        return;
      }
      ...
      if (this._isTextInput()) {
        ...
        this._caretMonitor = setInterval(function () {
          // Don't do anything if recorder doesn't exist
          if (!this.input) {
            return;
          }
    ...
    // Bind to blur event
    utils.Bind(ipt, "record:blur", function (e) {
      // Don't do anything if recorder doesn't exist
      if (!this.input) {
        return;
      }
    ...
    if (inputType != "checkbox" && inputType != "radio") {
      utils.Bind(ipt, "record:change", function () {
        // Don't do anything if recorder doesn't exist
        if (!this.input) {
          return;
        }
