...
(function () {
  ...
  Pop.prototype._getCXRParams = function () {
    ...
    if (this.cfg.replay === true && typeof _recordController !== 'undefined') {
      fstr += "cxrid=" + fs.enc(_recordController.getGlobalId()) + "&cxrurl=" + fs.enc(fs.config.recUrl);
    }
    return fstr;
  };
      ...
      if (this.cfg.preview) {
        ...
        url = fs.makeURI(
          [
            ...
            "&uid=", fs.enc(Singletons.stg.uid),
            "&ns=", fs.enc(fs.config.siteKey),
            "&brain=", fs.enc(fs.config.brainUrl),
            inf,
            "&", cxrStr
          ].join("")
        );
        ...
        utils
          .getBrainStorage(this.br, Singletons.stg.uid, this.cfg.preview ? "preview" : fs.config.siteKey)
          .set("fscfg", {
            gcfg: fs.enc(utils.Compress.compress(
              // extra fs.enc to make UTF8 survives compression
              fs.enc(JSON.stringify(generalConfig))
            )),
