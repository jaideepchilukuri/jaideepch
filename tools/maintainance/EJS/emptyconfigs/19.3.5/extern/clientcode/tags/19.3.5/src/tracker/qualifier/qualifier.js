...
fs.provide("track.qualifier.Qualifier");
...
(function () {
  ...
  var Qualifier = function (browser, cpps, data, qual, template, displayopts) {
    ...
    this.userLocale = this.data.def.language.locale || "en";
    ...
    if (this.userLocale !== "en") {
      if (fs.isDefined(this.data.def.qualifier.survey.locales[this.userLocale])) {
        this.qual.survey = fs.ext({}, this.data.def.qualifier.survey.locales[this.userLocale]);
      }
    }

    var questions, questionsLength = this.qual.survey.questions.length;
    ...
    this.disqualified.subscribe(fs.proxy(function () {
      this.showNoThanks();
    }, this));

    // Bind to validation failed
    this.validationFailed.subscribe(fs.proxy(function (msg) {
      /* pragma:DEBUG_START */
  ...
  Qualifier.prototype.render = function () {
    ...
    // Bind to buttons
    utils.Bind(document.getElementById('qualifierForm'), 'qualifier:submit', fs.proxy(function (e) {
      utils.preventDefault(e);
      this.validateAndSubmit();
    }, this));

    utils.Bind(document.getElementById('qualCancelButton'), 'qualifier:click', fs.proxy(function (e) {
      utils.preventDefault(e);
      this.disqualified.fire();
    }, this));

    utils.Bind(document.getElementById('qualCloseButton'), 'qualifier:click', fs.proxy(function (e) {
      utils.preventDefault(e);
      window.close();
    }, this));
  };
