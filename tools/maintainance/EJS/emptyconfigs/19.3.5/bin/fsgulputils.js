...
module.exports = ( function () {
     ...
     * Send the email
     */
    ctx.sendClientCodeEmail = function (cid, clientkey, toEmail, env, cb) {
      /**
       * The HTML templater
       * @param str
       * @param data
      ...
      var Templater = function (str, data) {
        ...
        }, mailConfig = pjson.email,
        data = {
          version: pjson.code_version,
          tstamp: (new Date()).getTime(),
          client: clientkey,
          env: env,
          envCap: capitalizeFirstLetter(env),
          cid: cid
        },
        embedSnip = fs.readFileSync('./bin/embedsnippet.html').toString();
      var embedBackup = embedSnip + '';
      var stagingSnip = entities.encode(embedSnip.replace('${JSURL}', "//gateway.foresee.com/sites/" + clientkey + "/staging/gateway.min.js")).replace(/\n/g, '<br>').replace(/[ ]{2}/g, '&nbsp;&nbsp;');
      var prodSnip = entities.encode(embedSnip.replace('${JSURL}', "//gateway.foresee.com/sites/" + clientkey + "/production/gateway.min.js")).replace(/\n/g, '<br>').replace(/[ ]{2}/g, '&nbsp;&nbsp;');
      data.stagingembed = stagingSnip;
      data.productionembed = prodSnip;
      var template = fs.readFileSync('./bin/emailtemplate.html').toString(),
        mailBody = Templater(template, data);
