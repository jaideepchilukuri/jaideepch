...
class Module {
  ...
  load() {
    ...
    /* pragma:DEBUG_START */
    console.log(`gw: Loading: ${this.name}`);
    /* pragma:DEBUG_END */
...
class Registry {
  ...
  _register(name, value) {
    if (!name) {
      ...
      /* pragma:DEBUG_START */
      console.log(`gw: Resolved ${name} using currentScript method`);
      /* pragma:DEBUG_END */
    }
  ...
  _legacyLoad(mod) {
    ...
    /* pragma:DEBUG_START */
    console.log(
      `gw: Legacy load resolved: ${mod.name} with ${pending.impl.toString().substring(0, 100)}`
    );
    /* pragma:DEBUG_END */
