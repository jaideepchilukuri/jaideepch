...
var projectsToBuild = {
  ...
  "mouseoff": { name: "mouseoff", deps: ["$fs.utils.js"] },
        ...
        var completePost = function (env) {
          ...
          fcp.postCodeVersion(buffer, rs.notes, pjson.version, rs.latest, function (success, data) {
            var envShort = Object
              ...
              .find(key => { return FcpClient.environments[key] === env });
...
gulp.task('push_default_config', ['_pullextern'], function (cb) {
  FcpClient.promptForFCPCredentials({}, function (rs) {
    ...
    if (rs.environment == FcpClient.environments.dev) {
      ...
      defCfg.modernSurveyUrl = defCfg.modernSurveyUrl.replace(/cxsurvey/, 'dev-cxsurvey');
      // defCfg.recUrl = defCfg.recUrl.replace(/record/, 'dev-record');
      ...
      defCfg.modernSurveyUrl = defCfg.modernSurveyUrl.replace(/cxsurvey/, 'qal-cxsurvey');
      // defCfg.recUrl = defCfg.recUrl.replace(/record/, 'qa-record');
