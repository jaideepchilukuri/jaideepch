        ...
        case "variables":
          for (j = 0; j < criteria.length; j++) {
            if (fs.isDefined(criteria[j].name)) {
              var ctx = _W,
                valbits = criteria[j].name.split('.'),
                k = 0;
              while (k < valbits.length && ctx) {
                ctx = ctx[valbits[k++]];
              }
              if (k == valbits.length && fs.isDefined(ctx)) {
                if (ctx == criteria[j].value) {
                  return true;
                } else if (ctx === true && criteria[j].value == "true") {
                  return true;
                } else if (ctx === false && criteria[j].value == "false") {
                  return true;
                }
              }
            }
