...
function bootSDK() {
  ...
  if (typeof _W["_fsAlreadyBootedSDK"] !== "undefined") {
    ...
    (function() {
      throw new Error(
        "ForeSee WebSDK detected its embed code was run multiple times! " +
          "Please ensure the embed snippet is only run once to avoid problems."
      );
    })();
    /* pragma:DEBUG_END */
    return;
                ...
                sanitize
                  ...
                  .then(passed => {
                    if (passed) {
                      ...
                      console.error("Foreseee WebSDK failed integrity code check on config.json.");
                      console.error(
